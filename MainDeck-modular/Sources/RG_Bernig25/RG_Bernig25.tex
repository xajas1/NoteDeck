...
\begin{DEF}{RG-B25-02-01}{n-dimensionale Untermannigfaltigkeit des euklidischen Raumes}
...
\begin{EXA}{RG-B25-02-02}{n-Sphäre}
Die $n$-Sphäre
$$
...
\begin{EXA}{RG-B25-02-03}{Hyperboloid}
$$
...
$$
\begin{note}
  Nach der Metrik-Kompatibilität ist die Ableitung der Metrik nach Vektorfeldern gleich der Produktregel, wobei in der Metrik die Ableitungen bzgl. des Zusammenhangs stehen.
\end{note}
Addiert man die ersten beiden Gleichungen, subtrahiert die dritte, und benutzt die Torsionsfreiheit, so ergibt sich
$$2 g\left(D_{X} Y, Z\right)=X g(Y, Z)+Y g(Z, X)-Z g(X, Y)+g([X, Y], Z)-g([X, Z], Y)-g([Y, Z], X)$$
\begin{note}
  Die Koszulgleichung entsteht durch Summation der ersten beiden Iterationen der Metrikkompatibilität $(X,Y,Z);(Y,Z,X)$ und Subtraktion der dritten Iteratioen $(Z,X,Y)$.
\end{note}
...
$$
\begin{note}
Koszulgleichung anwenden, dann Produktregel und Lie-Klammer Identität anwenden und $Y(f)g(\dots))$ Terme kürzen.
\end{note}

(c) $D_{X}(f Y)=X(f) Y+f D_{X} Y$.
\begin{note}
  Analog, Koszulgleichung anwenden, dann Produktregel und Lie-Klammer (mit $[X,fY]=f[X,Y]+X(f)Y$, Vorzeichen aufpassen)
\end{note}
...
\begin{note}
  Aus $C^\infty$-Linearität in $X$ und Ableitung in $Y$ des Zusammenhangs folgt
  $$D_XY=D_{X^i\partial_i}Y^j\partial_j= \cdots = (X^i\partial_iY^k+X^iY^j\Gamma_{ij}^k)\partial_k$$
  Durch Auswerten von Koszulgleichung für $\partial_i,\partial_j,\partial_l$ und durch anschließende Kontraktion mit invereser Metrik erhält man eine (lokale) Darstellung der Christoffel-Koeffizienten.  
\end{note}
\end{DEF} 
...
\begin{REM}{RG-B25-06-13}{Existenz von umgebenden Vektorfeldern}
...
\begin{PROP}{RG-B25-06-11}{Induzierter LC-Zusammenhang auf Untermannigfaltigkeiten von RMfk}
...
\begin{note}
  Man zeigt, dass der rechte Term genau die Eigenschaften des LC-Zusammenhangs erfüllt und verwendet dann Eindeutigkeit. Zuvor zeigt man die triviale Erweiterung bzw. Einschränkung von Ableitung von Funktionen längs Vektorfeldern und Lie-Klammern auf die umgebende Mannigfaltigkeit.
\end{note}
\end{PROOF}
...
&=\frac{\partial}{\partial t}
     \Bigl\langle\frac{\partial\alpha}{\partial u},
                 \frac{\partial\alpha}{\partial u}\Bigr\rangle
   =\frac{\partial}{\partial t}
     \Bigl(g_{ij}(\alpha)\,\alpha_{,u}^i\,\alpha_{,u}^j\Bigr)\\[2pt]
&=\bigl(\Gamma_{ki}^{\,s}g_{sj}+\Gamma_{kj}^{\,s}g_{is}\bigr)\,
      \alpha_{,t}^k\,\alpha_{,u}^i\,\alpha_{,u}^j
   +g_{ij}\,\alpha_{,tu}^i\,\alpha_{,u}^j
   +g_{ij}\,\alpha_{,u}^i\,\alpha_{,tu}^j\\[2pt]
&=2\,\alpha_{,u}^i\Bigl(
      g_{ij}\,\alpha_{,tu}^j
      +\Gamma_{kj}^{\,s}g_{si}\,
         \alpha_{,t}^k\,\alpha_{,u}^j\Bigr).
\end{aligned}
\]
...
\begin{note}
Warum dürfen die Ableitungen \(\alpha_{,t}^k\) und
\(\alpha_{,u}^j\) vertauscht werden? Setze
\(G_{kj}:=\Gamma_{kj}^{\,s}g_{si}\)  und
\(X_{kj}:=\alpha_{,t}^k\,\alpha_{,u}^j\).
Wegen der Torsionsfreiheit der Levi-Civita-Verbindung gilt
\(\Gamma_{kj}^{\,s}=\Gamma_{jk}^{\,s}\); somit ist \(G_{kj}=G_{jk}\) **symmetrisch**.

Schreibe \(X_{kj}\) als Summe aus symmetrischem und antisymmetrischem Teil:
\[
X_{kj}=S_{kj}+A_{kj},\qquad
S_{kj}:=\tfrac12\bigl(X_{kj}+X_{jk}\bigr),\quad
A_{kj}:=\tfrac12\bigl(X_{kj}-X_{jk}\bigr).
\]
Dann
\[
G_{kj}X_{kj}=G_{kj}S_{kj}+G_{kj}A_{kj}.
\]
Der zweite Summand verschwindet, weil ein symmetrischer Tensor
\(G_{kj}\) mit einem antisymmetrischen Tensor \(A_{kj}=-A_{jk}\)
kontrahiert immer Null ergibt:
\(
G_{kj}A_{kj}=0.
\)
Somit
\[
G_{kj}\,\alpha_{,t}^k\,\alpha_{,u}^j
   =\tfrac12G_{kj}\bigl(
        \alpha_{,t}^k\,\alpha_{,u}^j
       +\alpha_{,t}^j\,\alpha_{,u}^k\bigr)
   =G_{kj}\,\alpha_{,t}^j\,\alpha_{,u}^k,
\]
d.\,h. die beiden Ableitungen können innerhalb des Produkts beliebig
vertauscht werden.
\end{note}
\end{PROOF}
...
& = D_X D_Y U+I I\left(X, D_Y U\right)+X\left(\left\langle I I(Y, U), \nu_i\right\rangle\right) \nu_i+\left\langle I I(Y, U), \nu_i\right\rangle \tilde{D}_X \nu_i
\end{aligned}
$$
und somit
$$\left\langle\tilde{D}_X \tilde{D}_Y U, V\right\rangle=\left\langle D_X D_Y U, V\right\rangle+\left\langle I I(Y, U), \nu_i\right\rangle\left\langle\tilde{D}_X \nu_i, V\right\rangle$$
...
$$
\begin{aligned} \tilde{R}(X, Y, U, V)= & \left\langle\tilde{D}_Y \tilde{D}_X U, V\right\rangle-\left\langle\tilde{D}_X \tilde{D}_Y U, V\right\rangle+\left\langle\tilde{D}_{[X, Y]} U, V\right\rangle \\ = & \left\langle D_Y D_X U, V\right\rangle+\left\langle I I(X, U), \nu_i\right\rangle\left\langle\nu_i, I I(Y, V)\right\rangle \\ & -\left\langle D_X D_Y U, V\right\rangle-\left\langle I I(Y, U), \nu_i\right\rangle\left\langle\nu_i, I I(X, V)\right\rangle \\ & +\left\langle D_{[X, Y]} U, V\right\rangle \\ = & R(X, Y, U, V)-\left\langle I I(X, U), \nu_i\right\rangle\left\langle\nu_i, I I(Y, V)\right\rangle+\left\langle I I(Y, U), \nu_i\right\rangle\left\langle\nu_i, I I(X, V)\right\rangle \\ = & R(X, Y, U, V)-\langle I I(X, U), I I(Y, V)\rangle+\langle I I(Y, U), I I(X, V)\rangle .\end{aligned}
$$
...
$$
\begin{aligned}
\frac{d}{d t} L\left(c_{t}\right) & =\frac{d}{d t} \int_{0}^{L} g\left(c_{t}^{\prime}(s), c_{t}^{\prime}(s)\right)^{\frac{1}{2}} d s \\
...
\begin{DEF}{RG-B25-23-01}{Konstruktion des komplex-projektiven Raumes}
...
\begin{LEM}{RG-B25-23-02}{Submersion von $S^{2n+1}$ in komplex-projektiven Raum und Horizontal- / Vertikalräume}
...
\begin{PROOF}{RG-B25-23-03}{P: Submersion von $S^{2n+1}$ in komplex-projektiven Raum und Horizontal- / Vertikalräume}
Wir wissen bereits, dass die Projektion $\pi: S^{2 n+1} \rightarrow \mathbb{C} \mathbb{P}^{n}$ eine Submersion ist (siehe Satz 1.8). Wir möchten einen direkten Beweis dafür geben.
...
\begin{CONC}{RG-B25-23-04}{Fubini-Study-Metrik auf $\mathbb{C P}^{n}$}
...
\begin{CONC}{RG-B25-23-05}{Fast komplexe Struktur auf $\mathbb{C P}^{n}$}
...
\begin{THEO}{RG-B25-23-06}{Schnittkrümmung von Ebenen in $\mathbb{C P}^{n}$}
...
\begin{PROOF}{RG-B25-23-07}{P: Schnittkrümmung von Ebenen in $\mathbb{C P}^{n}$}
Wir betrachten die Geodätischen
...
Es folgt
\begin{equation*}
Y(s)=d \pi(\tilde{Y}(s))=\sin (s) \underbrace{d \pi(\tilde{U}(s))}_{=: U(s)}
\end{equation*}


\textbf{Fall 1}: $\tilde{u} \perp i \tilde{v}$. Dann ist $\left\langle\tilde{U}(s), \tilde{c}^{\prime}(s)\right\rangle=0$ und $\left\langle\tilde{U}(s), i \tilde{c}^{\prime}(s)\right\rangle=0$ für alle $s$.
...
Jetzt leiten wir $Y(s)=\sin (s) U(s)$ nach $s$ ab und finden 

$$Y^{\prime}(s)=\cos (s) U(s) \text{ und } Y^{\prime \prime}(s)=-\sin (s) U(s)=-Y(s)$$
...
\textbf{2. Fall}: $\tilde{u}=i \tilde{v}$ (d.h. $u=J v$ ). Dann ist $\tilde{U}(s)=i \tilde{v}$ für alle $s$.
...
Es folgt
\begin{equation*}
Y(s)=d \pi(\tilde{Y}(s))=\sin (s) d \pi(\tilde{U}(s))=\cos (s) \sin (s) J c^{\prime}(s)
\end{equation*}
...
\begin{LEM}{RG-B25-24-01}{Darstellung von Vektorfeldern längs Kurven durch parallele Vektorfelder}
...
\begin{PROOF}{RG-B25-24-02}{P: Darstellung von Vektorfeldern längs Kurven durch parallele Vektorfelder}
Seien $E_{1}, \ldots, E_{n}$ eine Basis paralleler Vektorfelder längs c. Dann können wir $Y$ schreiben als
$$
...
Theorem 4.2 (Längen kleiner Kreise). 


\begin{THEO}{RG-B25-24-03}{Längen kleiner Kreise}
...
\begin{PROOF}{RG-B25-24-04}{P: Längen kleiner Kreise}
Sei $Y_{\theta}$ das Jacobifeld entlang der Geodätischen $t \mapsto H(t, \theta)$, welches durch die geodätische Variation $H$ definiert wird. Dann gilt $\frac{\partial}{\partial \theta} H(r, \theta)=Y_{\theta}(r)$ und somit
$$
...
$$L\left(C_{r}\right)=\int_{0}^{2 \pi} r\left(1-\frac{r^{2}}{6} K(P)+o\left(r^{2}\right)\right) d \theta=2 \pi$$
\end{PROOF}
...
\begin{KORO}{RG-B25-24-05}{Abschätzung für Schnittkrümmung unter Submersion}
...
\begin{PROOF}{RG-B25-24-06}{P: Abschätzung für Schnittkrümmung unter Submersion}
Die Geodätischen $r \mapsto \exp _{\tilde{m}} r(\cos (\theta) \tilde{u}+\sin (\theta) \tilde{v})$ sind horizontal und projizieren nach Theorem 2.2 auf die Geodätischen $r \mapsto$ $\exp _{m} r(\cos (\theta) u+\sin (\theta) v)$. Es folgt $\pi\left(\tilde{C}_{r}\right)=C_{r}$ für alle $r$. Da $\pi$ Längen verkleinert (siehe Theorem 2.2), gilt $L\left(C_{r}\right)=L\left(\pi\left(\tilde{C}_{r}\right)\right) \leq L\left(\tilde{C}_{r}\right)$. Setzt man die eben bewiesenen Asymptotiken ein, so folgt
$$
...
\begin{LEM}{RG-B25-20-12}{Optimalität von geraden Strecken unter Exponentialabbildung}
...
\begin{PROOF}{RG-B25-20-14}{P: Optimalität von geraden Strecken unter Exponentialabbildung}
Das folgt aus dem Beweis von Theorem 3.4, 2. Fall.
\end{PROOF}


Definition 5.2. 

\begin{DEF}{RG-B25-20-15}{Konjugierte Punkte entlang von Geodätischen}
Sei $c:[a, b] \rightarrow M$ eine Geodätische, $p=c(a), q=$ $c(b)$. Dann heissen $p$ und $q$ konjugiert längs $c$, falls es ein nichttriviales Jacobifeld $Y$ längs $c$ gibt mit $Y(a)=0, Y(b)=0$.
\end{DEF}

Lemma 5.3. 


\begin{LEM}{RG-B25-20-16}{Charakterisierung von konjungierten Punkten durch singuläre Differentiale}
...
\begin{PROOF}{RG-B25-20-17}{P: Charakterisierung von konjungierten Punkten durch singuläre Differentiale}
Nach Korollar 6.5 gilt
$$
...
\begin{THEO}{RG-B25-20-18}{Charakterisierung von lokal minimierenden Geodätischen durch konjugierte Punkte}
...
\begin{PROOF}{RG-B25-20-19}{P: Charakterisierung von lokal minimierenden Geodätischen durch konjugierte Punkte}
...
\begin{EXA}{RG-B25-20-20}{Global minimierende Geodätische auf Sphäre}
Ist $M=S^{n}$ mit der Standardmetrik, so gibt es für Geodätische der Länge $<\pi$ keine konjugierten Punkte. Diese sind in der Tat lokal, sogar global minimierend.
\end{EXA}


\begin{EXA}{RG-B25-20-21}{Lokal minimierende Geodätische auf Torus}
Ist $M=T^{n}$ der Torus, so gibt es für keine Geodätische konjugierte Punkte: die horizontalen Lifts von Geodätischen unter der Riemannschen Submersion $\mathbb{R}^{n} \rightarrow T^{n}$ sind einfach Geraden in $\mathbb{R}^{n}$ und es gibt keine Variation einer Geraden, die zwei Punkte festlässt. Es folgt, dass jede Geodätische in $T^{n}$ lokal minimal ist (d.h. jede Kurve $\gamma$ nahe $c$ hat grössere Länge). Diese müssen natürlich nicht global minimierend sein.
\end{EXA}
...
\begin{PROP}{RG-B25-25-01}{Lokale Isometrie von geodätisch vollständigem Raum ist eine Überlagerung}
Sei $\pi:(\tilde{M}, \tilde{g}) \rightarrow(M, g)$ eine lokale Isometrie. Ist $(\tilde{M}, \tilde{g})$ geodätisch vollständig, so ist $\pi$ eine Überlagerungsabbildung.
\end{PROP}

Beweis. 

\begin{PROOF}{RG-B25-25-02}{P: Lokale Isometrie von geodätisch vollständigem Raum ist eine Überlagerung}
Sei $m \in M$ und $\pi^{-1}(m)=\left\{\tilde{m}_{i}, i \in I\right\}$. Sei $r$ klein genug, so dass $\exp _{m}: B\left(0_{m}, r\right) \rightarrow B(m, r)$ ein Diffeomorphismus ist. Setze $U_{i}:=B\left(\tilde{m}_{i}, r\right), U:=B(m, r)$. Da ( $\tilde{M}, \tilde{g}$ ) geodätisch vollständig ist, gilt nach dem Satz von Hopf-Rinow
$$
...
\begin{DEF}{RG-B25-25-03}{Einfach zusammenhängende Mannigfaltigkeite}
...
\begin{LEM}{RG-B25-25-04}{Überlagerung von ZSH Raum in EZSH Raum ist ein Diffeomorphismus}
...
\begin{PROOF}{RG-B25-25-05}{P: Überlagerung von ZSH Raum in EZSH Raum ist ein Diffeomorphismus}
Jede Kurve $\gamma$ in $M$ kann zu einer Kurve $\tilde{\gamma}$ in $\tilde{M}$ geliftet werden. Ist der Anfangspunkt $\tilde{\gamma}(0) \in \pi^{-1}(\gamma(0))$ vorgegeben, so ist der Lift eindeutig bestimmt.
...
Theorem 6.3 (Mannigfaltigkeiten konstanter Schnittkrümmung). 

\begin{THEO}{RG-B25-25-06}{Klassifikation von EZSH vollständigen Mannigfaltigkeiten mit konstanter Schnittkrümmung}
Sei ( $M, g$ ) eine vollständige, einfach zusammenhängende Riemannsche Mannigfaltigkeit mit konstanter Schnittkrümmung. Dann ist ( $M, g$ ) isometrisch zu $\mathbb{R}^{n}$, falls $K \equiv 0$; zu ( $\mathbb{H}^{n}$, can), falls $K \equiv-1$ und zu ( $S^{n}$, can), falls $K \equiv 1$.
...
\begin{PROOF}{RG-B25-25-07}{P: Klassifikation von EZSH vollständigen Mannigfaltigkeiten mit konstanter Schnittkrümmung}
Sei $m \in M, v \in T_{m} M$ und setze $c(t):=\exp _{m}(t v)$. Seien $u_{1}, u_{2} \in T_{m} M$ und sei $U_{i}$ das parallele Vektorfeld längs $c$ mit $U_{i}(0)=$ $u_{i}, i=1,2$. Sei $Y_{i}$ das Jacobifeld längs $c$ mit $Y_{i}(0)=0, Y_{i}^{\prime}(0)=u_{i}$. Nach Korollar 6.5 gilt
$$
...
(3) Sei $K \equiv 1$. Dann ist $Y_{i}(t)=\sin (t) U_{i}(t)$. Ist $\tilde{m} \in S^{n}$, so ist $\exp _{\tilde{m}}: B(0, \pi) \subset T_{\tilde{m}} S^{n} \rightarrow S^{n} \backslash\{-\tilde{m}\}$ ein Diffeomorphismus. Also gibt es wie im vorigen Fall eine lokale Isometrie $F$ von $S^{n} \backslash\{-\tilde{m}\}$ nach $M$. Das zugehörige Diagramm ist
\[
\begin{tikzcd}
M & S^n \setminus \{-\tilde{m}\} \arrow[l, "F"'] \\
T_m M \arrow[u, "\exp_m"] \arrow[r, "f"'] 
& T_{\tilde{m}} S^n \arrow[u, "\exp_{\tilde{m}}"']
\end{tikzcd}
\]
...
Es ist $F_{1}\left(\tilde{m}^{\prime}\right)=m^{\prime}=F\left(\tilde{m}^{\prime}\right)$ und 
$$\left.d F_{1}\right|_{\tilde{m}^{\prime}}=\left.\left.d \exp _{m^{\prime}}\right|_{0} \circ d F\right|_{\tilde{m}^{\prime}} \circ\left(\left.\left(d \exp _{\tilde{m}}\right)\right|_{0}\right)^{-1}=\left.d F\right|_{\tilde{m}^{\prime}}$$
Sei $\tilde{c}(t), 0 \leq t<\pi$ eine Geodätische auf $S^{n}$ mit $\tilde{c}(t)=\tilde{m}^{\prime}$, die nicht durch $-\tilde{m}$ geht. Da $F$ und $F_{1}$ lokale Isometrien sind, sind $c(t):=F(\tilde{c}(t))$ und $c_{1}(t):=F_{1}(\tilde{c}(t))$ Geodätische. Es gilt aber
$$c_{1}(0)=F(\tilde{c}(0))=F\left(\tilde{m}^{\prime}\right)=F_{1}\left(\tilde{m}^{\prime}\right)=c_{1}(0)$$ 
und
$$
...
\begin{REM}{RG-B25-25-08}{Verallgemeinerung der Klassifikation für nicht EZSH Mannigfaltigkeiten}
Ist ( $M, g$ ) nicht einfach zusammenhängend, so gibt es eine einfach zusammenhängende Überlagerung $(\tilde{M}, \tilde{g}) \rightarrow(M, g)$ (die sogenannte universelle Überlagerung), auf welche das Theorem angewendet werden kann. Es kann daher auch wie folgt formuliert werden:
\end{REM}

Theorem 6.4. 

\begin{THEO}{RG-B25-25-09}{Klassifikation der universellen Überlagerung von vollständigen Mannigfaltigkeiten mit konstanter Schnittkrümmung}
Ist ( $M, g$ ) eine vollständige Riemannsche Mannigfaltigkeit mit konstanter Schnittkrümmung, so ist die universelle Überlagerung $\mathbb{R}^{n}$, falls $K \equiv 0 ; \mathbb{H}^{n}$, falls $K \equiv-1$ und $S^{n}$, falls $K \equiv 1$.
\end{THEO}

\begin{EXA}{RG-B25-25-10}{Anwendung der Klassifikation für $\mathbb{R P}^{n}$}
...
\subsection{Der Schnittort}


\begin{CONC}{RG-B25-26-01}{Setting für Schnittort}
...
\begin{DEF}{RG-B25-26-02}{Schnittort von Punkten einer Mannigfaltigkeit}
Der Schnittort von $m$ ist
$$
...
\begin{REM}{RG-B25-26-03}{Charakterisierung von $v\in\partial U$}
Für $v \in \partial U_{m}$ gibt es (nur) zwei Gründe, die auch simultan auftreten können. Der erste Grund ist, dass es eine weitere minimale Geodätische zwischen $c_{v}(0)$ und $c_{v}(1)$ gibt. Der zweite Grund ist, dass $c_{v}(0)$ und $c_{v}(1)$ konjugiert sind. Dann kann eine Fortsetzung von $c_{v}$ über den Punkt $c_{v}(1)$ hinaus nicht minimal sein nach Theorem 5.4.
\end{REM}

Satz 1.2. 


\begin{PROP}{RG-B25-26-04}{Disjunkte Zerlegung von Mannigfaltigkeit bzgl Schnittort}
Die Mannigfaltigkeit $M$ lässt sich disjunkt zerlegen in
$$
...
\begin{PROOF}{RG-B25-26-05}{P: Disjunkte Zerlegung von Mannigfaltigkeit bzgl Schnittort}
Sei $x \in M$. Dann gibt es eine minimale Geodätische $c_{v}$ von $m$ nach $x$. Ohne Einschränkung sei $c_{v}$ auf $[0,1]$ parametrisiert. Dann ist $v \in \overline{U_{m}}=U_{m} \cup \partial U_{m}$. Es folgt $M=\exp _{m}\left(U_{m}\right) \cup \operatorname{Cut}(m)$.
...
\begin{EXA}{RG-B25-26-06}{Schnittort auf der Sphäre}
Der Schnittort eines Punktes auf der Sphäre ist der antipodale Punkt. Der Schnittort eines Punktes $m$ des $n$-dimensionalen reell-projektiven Raumes besteht aus den zu $m$ senkrechten Punkten, diese bilden einen ( $n-1$ )-dimensionalen reell-projektiven Raum.
\end{EXA}
...
\begin{DEF}{RG-B25-27-02}{Dichte auf glatten Mannigfaltigkeiten}
Eine Dichte auf einer glatten Mannigfaltigkeit M, versehen mit einem Atlas $\left(U_{i}, \phi_{i}\right)$, ist gegeben durch ein Maß $\mu_{i}$ auf $\phi_{i}\left(U_{i}\right)$ mit folgenden Eigenschaften:
...
\begin{EXA}{RG-B25-27-03}{Volumenformen definieren Dichten auf Mannigfaltigkeiten}
Jede Volumenform (d.h. nirgends verschwindende $n$-Form) definiert eine Dichte auf $M$.
\end{EXA}



\begin{CONC}{RG-B25-27-04}{Maß aus Dichte auf Mannigfaltigkeit}
...
\begin{REM}{RG-B25-27-05}{Beziehung zwischen Dichten}
...
\begin{DEF}{RG-B25-27-06}{Nullmengen bzgl Dichten auf Mannigfaltigkeiten}
Eine Teilmenge $A \subset M$ hat Maß 0 (oder ist eine Nullmenge), wenn $\delta(A)=0$ für eine (und dann für jede) Dichte $\delta$ auf M.
\end{DEF}



\begin{DEF}{RG-B25-27-07}{Volumendichte auf vollständigen Riemannischen Mannigfaltigkeiten}
...
$$
Diese Familie erfüllt die Bedingungen einer Dichte auf $M$. Hierzu benutzt man einen Variablenwechsel auf $\mathbb{R}^{n}$, der Faktor $\sqrt{\operatorname{det}\left(g_{i j}\right)}$ ist genau so gewählt, dass diese Unabhängigkeit gilt. Die so definierte Dichte ist die kanonische Volumendichte auf $(M, g)$ und wird mit $v_{g}$ bezeichnet.
\end{DEF}



\begin{DEF}{RG-B25-27-08}{Volumen einer Riemannischen Mannigfaltigkeit}
...
\begin{EXA}{RG-B25-27-09}{Volumen auf $\R^2$ mit Metrik}
...
\begin{LEM}{RG-B25-27-10}{Schnittorte sind Nullmengen}
Für jedes $m \in M$ ist der Schnittort Cut $(m)$ eine Nullmenge.
\end{LEM}
...
\begin{PROOF}{RG-B25-27-11}{Schnittorte sind Nullmengen}
Es ist $\operatorname{Cut}(m)=\exp _{m}\left(\partial U_{m}\right)$. Da $\partial U_{m}$ eine Nullmenge in $T_{m} M$ ist und $\exp _{m}$ stetig ist, folgt die Behauptung.
\end{PROOF}
...
\begin{CONC}{RG-B25-27-12}{Berechnung von Volumen ausgehend von einem Punkt}
...
\begin{equation*}
\operatorname{vol}(M, g)=\int_{S^{n-1}} \int_{0}^{\rho(u)} J(u, t) t^{n-1} d t d u
\end{equation*}
Man beachte, dass $\lim _{t \rightarrow 0} J(u, t)=1$ für alle $u$.
\end{CONC}


% ——————————————————————————————————————————————————————————
% Volumen über geodätische Polarkoordinaten (Erklärung + Beispiele)
% ——————————————————————————————————————————————————————————

% (A) Begriffe / Notation
\paragraph*{Begriffe und Notation.}
Sei $(M,g)$ eine vollständige Riemannsche Mannigfaltigkeit, $m\in M$ fest.
\begin{itemize}
  \item \emph{Exponentialabbildung:} $\exp_m:T_mM\to M$, $v\mapsto c_v(1)$, wobei $c_v$ die Geodätische mit $c_v(0)=m$, $c_v'(0)=v$ ist (Bogenlänge als Zeit).
  \item \emph{Schnittort / Cut locus:} $Cut(m)\subset M$ ist die Menge der Endpunkte, an denen die minimierende Geodätische von $m$ aus \emph{aufhört} eindeutig minimierend zu sein. Klassisch: $Cut(m)$ hat Maß $0$.
  \item \emph{Geodätische Polarkoordinaten:} Schreibe $v=tu$ mit $t\ge 0$ und $u\in S^{n-1}\subset T_mM$ (Einheitssphäre bzgl.\ $g_m$). Setze $c(t)=\exp_m(tu)$.
  \item \emph{Jacobi-Felder:} Für $i=2,\dots,n$ seien $Y_i$ Jacobi-Felder längs $c$ mit $Y_i(0)=0$ und $Y'_i(0)=e_i$, wobei $\{u,e_2,\dots,e_n\}$ ONB in $T_mM$ ist.
  \item \emph{Volumenform:} $v_g$ ist die Riemannsche Volumenform; $\exp_m^*v_g$ ist ihr Pullback auf $T_mM$.
  \item \emph{Jacobianfaktor:}
  \[
    J(u,t)\;:=\;t^{-(n-1)}\sqrt{\det\big(g(Y_i(t),Y_j(t))_{i,j=2}^{n}\big)}.
  \]
  Man beachte $J(u,t)\to 1$ für $t\to 0$.
\end{itemize}

% (B) Schritt-für-Schritt-Herleitung
\paragraph*{Schritt-für-Schritt-Herleitung.}
\begin{enumerate}
  \item \textbf{Hopf–Rinow \& Zerlegung.}
  Vollständigkeit $\Rightarrow$ jedes $x\in M$ ist durch eine \emph{minimierende} Geodätische mit $m$ verbunden. Außerhalb der $Cut(m)$ ist diese minimierende Geodätische eindeutig.
  Daher ist $M=\exp_m(U_m)\,\cup\,Cut(m)$ mit einem sternförmigen $U_m\subset T_mM$ und $vol(Cut(m))=0$.
  \[
    vol(M,g)=\int_{U_m}\exp_m^*v_g.
  \]

  \item \textbf{Geodätische Polarkoordinaten in $T_mM$.}
  Schreibe $v=tu$ (mit $u\in S^{n-1}$, $t\ge 0$). Das euklidische Maß in $T_mM\simeq\mathbb{R}^n$ zerfällt als
  \[
     dx_1\cdots dx_n \;=\; t^{\,n-1}\,dt\,du,
  \]
  wobei $du$ das kanonische Maß auf $S^{n-1}$ ist.

  \item \textbf{Differential der Exponentialabbildung.}
  Für die Richtung $u$ gilt $\big(d\exp_m\big)_{tu}(u)=c'(t)$ (Tangentenabbildung entlang der Geodätischen).
  Variiere $u$ in Richtung $e_i$ via $\sigma_i(s,t):=\exp_m\big(t(u+se_i)\big)$;
  das Jacobi-Feld $Y_i(t):=\partial_s|_{s=0}\sigma_i(s,t)$ erfüllt
  \[
     \big(d\exp_m\big)_{tu}(e_i)=\frac{1}{t}\,Y_i(t),\qquad i=2,\dots,n.
  \]

  \item \textbf{Jacobian von $\exp_m$ in der Basis $(u,e_2,\dots,e_n)$.}
  Das Bild der Basis ist $\big(c'(t),\,t^{-1}Y_2(t),\dots,t^{-1}Y_n(t)\big)$.
  Da $|c'(t)|=1$ (Bogenlänge), ergibt sich für den Betrag der Jacobimatrix
  \[
     \big|\det d\exp_m\big|_{tu}
     \;=\;t^{-(n-1)}\sqrt{\det\big(g(Y_i(t),Y_j(t))\big)} \;=\; J(u,t).
  \]

  \item \textbf{Pullback der Volumenform.}
  Damit gilt lokal (in Polarkoordinaten) die „Wechsel-der-Variablen“-Formel
  \[
     \exp_m^*v_g \;=\; J(u,t)\,dx_1\cdots dx_n \;=\; J(u,t)\,t^{n-1}\,dt\,du.
  \]

  \item \textbf{Integrationsgrenzen.}
  In Richtung $u$ kann man längs $c(t)$ genau bis zum (ersten) Schnittpunkt gehen; setze dafür
  $\rho(u)\in(0,\infty]$ als die Entfernung von $m$ zum Schnittort in Richtung $u$.
  Integriert man Radien $t\in[0,\rho(u)]$ und alle Richtungen $u\in S^{n-1}$, folgt
  \[
     vol(M,g)\;=\;\int_{S^{n-1}}\!\int_{0}^{\rho(u)} J(u,t)\;t^{\,n-1}\,dt\,du.
  \]

  \item \textbf{Verhalten bei $t\to 0$.}
  Aus der Jacobi-Gleichung erhält man $Y_i(t)=t\,e_i+O(t^3)$, also
  $\det(g(Y_i,Y_j))=t^{2(n-1)}\big(1+O(t^2)\big)$ und damit $J(u,t)\to 1$.
\end{enumerate}

% (C) Beispiele
\paragraph*{Beispiele.}

\textbf{(1) Euklidischer Raum $(\mathbb{R}^n, g_{\mathrm{eucl}})$.}
Hier sind Geodätische Geraden, $Y_i(t)=t\,e_i$, also $g(Y_i,Y_j)=t^2\delta_{ij}$ und $J(u,t)=1$.
Für die Volumenformel eines \emph{Geodätischen Balls} $B_R(m)$ erhält man
\[
  vol\big(B_R(m)\big)
   \;=\;\int_{S^{n-1}}\!\int_{0}^{R} 1\cdot t^{n-1}\,dt\,du
   \;=\;\big|S^{n-1}\big|\cdot \frac{R^n}{n},
\]
mit $\lvert S^{n-1}\rvert=\dfrac{2\pi^{\frac{n}{2}}}{\Gamma(\frac{n}{2})}$.

\medskip
\textbf{(2) Runde $2$-Sphäre $(S^2,g_{\mathrm{rd}})$.}
Wähle $m$ als Nordpol. Jede Geodätische ist ein Großkreis; der Schnittort ist der Antipode, also $\rho(u)=\pi$.
Orthogonale Jacobi-Felder erfüllen $Y''+Y=0$ mit $Y(0)=0$, $Y'(0)=e_i$, d.\,h.\ $Y_i(t)=\sin(t)\,E_i(t)$.
Damit $g(Y_i,Y_j)=\sin^2(t)\,\delta_{ij}$ und
\[
  J(u,t)
  \;=\;\frac{\sin t}{t}.
\]
...
\[
  \mathrm{Area}(S^2)
  \;=\;\int_{S^{1}}\!\int_{0}^{\pi} \frac{\sin t}{t}\,t\,dt\,du
  \;=\;|S^{1}|\int_{0}^{\pi}\sin t\,dt
  \;=\;2\pi\,[\,{-}\cos t\,]_{0}^{\pi}
  \;=\;4\pi,
\]
wie erwartet.
...
% (D) Bemerkung: Klein-t-Entwicklung
\paragraph*{Bemerkung (Klein-$t$-Entwicklung).}
Man erhält die klassische Expansion
\[
  J(u,t)\;=\;1-\frac{1}{6}\,\mathrm{Ric}_m(u,u)\,t^2+O(t^3),
\]
die den Einfluss der Krümmung auf das „Volumenmaß in Polarkoordinaten“ beschreibt.
...
\begin{EXA}{RG-B25-27-13}{Volumenberechnung auf der Sphäre}
Für die Sphäre $S^{n}$ mit der kanonischen Metrik gilt $Y_{i}(t)=$ $\sin (t) E_{i}(t)$ mit $E_{i}$ parallelen Vektorfeldern, $\rho(u)=\pi$ und somit
$$
\begin{aligned}
...
\begin{EXA}{RG-B25-27-14}{Volumenberechnung des hyperbolischen Raumes}
Das Volumen des hyperbolischen Raumes ist unendlich. Für $R>0$ ergibt sich wie für die Sphäre
$$
...
\begin{EXA}{RG-B25-27-15}{Volumenberechnung des komplex-projektiven Raumes}
Für den komplex-projektiven Raum wähle eine orthonormale Basis $\left\{u, e_{2}, \ldots\right\}$ von $T_{m} \mathbb{C}^{n}$ mit $e_{2}=J u$. Dann sind nach (9) und (10) die Jacobifelder gegeben durch
$$
\begin{aligned}
...
\subsection{Volumen und Krümmung}
...
\begin{LEM}{RG-B25-27-16}{Variation der Determinante}
Sei $A(t)$ eine glatte Kurve von $n \times n$-Matrizen mit $A(0)=$ Id. Dann gilt
$$
...
\begin{PROOF}{RG-B25-27-17}{P: Variation der Determinante}
Per Induktion über $n$. Ist $n=1$, so ist die Aussage trivial. Für $n>1$ entwickeln wir $\operatorname{det} A$ nach der ersten Zeile und bekommen
$$
...
\begin{EXA}{RG-B25-27-18}{Volumen Darstellung von Integral über symmetrische Bilinearform auf $\R^n$}
Sei $\phi$ eine symmetrische Bilinearform auf $\mathbb{R}^{n}$. Dann gilt
$$
...
\begin{PROOF}{RG-B25-27-19}{P: Volumen Darstellung von Integral über symmetrische Bilinearform auf $\R^n$}
Seien $v_{1}, \ldots, v_{n}$ die Koordinaten von $v$. Dann ist $\phi(v, v)=$ $\sum_{i, j} v_{i} v_{j} \phi\left(e_{i}, e_{j}\right)$, also
$$
...
Theorem 3.3 (Volumen kleiner Bälle). 


\begin{THEO}{RG-B25-27-20}{Darstellung des Volumen kleiner Bälle durch Skalarkrümmung}
Sei $m$ ein Punkt von $(M, g)$. Dann gilt
$$
...
\begin{PROOF}{RG-B25-27-21}{P: Darstellung des Volumen kleiner Bälle durch Skalarkrümmung}
Für $r$ kleiner als der Abstand zum Schnittort gilt
$$
...
$$
Insbesondere ist $Y_{i}(0)=0, Y_{i}^{\prime}(0)=e_{i}, Y_{i}^{\prime \prime}(0)=0$. Wir benötigen einen weiteren Term. Dazu leiten wir die Jacobigleichung nach $s$ ab und setzen $s=0$. Da $Y_{i}(0)=0$ spielt dabei nur der Term eine Rolle, bei dem $Y_{i}(s)$ abgeleitet wird (bei den anderen Termen wird entweder $R$ oder $c^{\prime}(s)$ abgeleitet). Es folgt
$$
...
\begin{DEF}{RG-B25-27-22}{Notation für Volumen eines $r$-Balls in vollständigen EZSH Mannigfaltigkeiten mit konstanter Schnittkrümmung}
...
Theorem 3.4 (Bishop-Gunther). 


\begin{THEO}{RG-B25-27-23}{Bishop-Gunther Theorem}
Sei ( $M, g$ ) eine vollständige Riemannsche Mannigfaltigkeit und $B_{m}(r)$ ein Ball, der den Schnittort von $m$ nicht trifft.
...
\begin{PROOF}{RG-B25-27-24}{P: Bishop-Gunther Theorem}
(1) Zunächst eine Vorbemerkung. Sei $c$ eine nach Bogenlänge parametrisierte Geodätische und $Y_{i}, i=2, \ldots, n$ Jacobifelder längs $c$ mit $Y_{i}(0)=0$, so dass $c^{\prime}(0), Y_{2}^{\prime}(0), \ldots, Y_{n}^{\prime}(0)$ eine Basis (aber nicht unbedingt Orthonormalbasis) von $T_{m} M$ bilden.
...
$$
Nach Voraussetzung an die Riccikrümmung ist das Integral negativ. Aus (12) folgt
$$
\begin{aligned}
...
$$\log f(s)=\left.\log f(r)\right|_{0} ^{s}=\int_{0}^{s} \frac{f^{\prime}(r)}{f(r)} d r \leq \int_{0}^{s} \frac{f_{a}^{\prime}(r)}{f_{a}(r)} d r=\left.\log f_{a}(r)\right|_{0} ^{s}=\log f_{a}(s)$$ 
also $f(s) \leq f_{a}(s)$.
...
\subsection{Krümmung und Wachstum der Fundamentalgruppe}



\begin{DEF}{RG-B25-28-01}{Endliche Darstellung von Gruppen und Anzahl der Wörter mit Länge kleiner gleich}
...
$$
Die Zahl $l_{S}(g)=\sum_{i}\left|r_{i}\right|$ heisst Länge von $g$. Diese hängt nicht nur von $g$, sondern von der Wahl der Menge $S$ der Generatoren ab. 

Mit $\phi_{S}(s)$ wird die Anzahl der Elemente von $\Gamma$ mit Länge kleiner gleich $s$ bezeichnet.
\end{DEF}
...
\begin{DEF}{RG-B25-28-02}{Wachstum von erzeugten Gruppen}
\begin{enumerate}
    \item Eine endlich erzeugte Gruppe hat \emph{exponentielles Wachstum}, wenn es $a > 0$ gibt mit $\phi_S(s) \geq \exp(a s)$ für alle $s$.
    \item Eine endlich erzeugte Gruppe hat \emph{polynomielles Wachstum vom Grad $\leq n$}, wenn es $a > 0$ gibt mit $\phi_S(s) \leq a s^n$ für alle $s$.
    \item Eine endlich erzeugte Gruppe hat \emph{polynomielles Wachstum vom Grad $n$}, falls sie polynomielles Wachstum vom Grad $\leq n$ hat, aber nicht vom Grad $\leq (n-1)$.
\end{enumerate}
\end{DEF}


\begin{REM}{RG-B25-28-03}{Unabhängigkeit des Wachstums von Wahl des Erzeugendensystems}
...
Theorem 4.2 (Milnor-Wolf). 


\begin{THEO}{RG-B25-28-04}{Milnor-Wolf Theorem}
Sei ( $M, g$ ) eine vollständige Riemannsche Mannigfaltigkeit mit nichtnegativer Riccikrümmung. Dann hat jede endlich erzeugte Untergruppe $\Gamma$ von $\pi_{1}(M)$ polynomielles Wachstum vom Grad $\leq n$ (Ist $M$ kompakt, so ist $\pi_{1}(M)$ bereits endlich erzeugt).
\end{THEO}
...
\begin{PROOF}{RG-B25-28-05}{P: Milnor-Wolf Theorem}
Sei ( $\tilde{M}, \tilde{g}$ ) die universelle Riemannsche Überlagerung und $a \in \tilde{M}$. Die Fundamentalgruppe $\pi_{1}(M)$ operiert auf $\tilde{M}$.
