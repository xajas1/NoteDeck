...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-005}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-006}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-007}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-008}
 
\caption{Abbildung 2.1: Kartenwechsel}
\end{center}
\end{figure}

Definition 2.1 (Karten, Atlanten, komplexe Strukturen) Sei $X$ ein topologischer Raum. Eine Karte $(U, h)$ von $X$ ist ein Homöomorphismus $h: U \rightarrow h(U) \subseteq \mathbb{C}$ von einer offenen Teilmenge $U \subseteq X$ auf eine offene Teilmenge von $\mathbb{C}$. Ein Atlas $A=\left\{\left(U_{i}, h_{i}\right)\right\}$ von $X$ ist eine Menge von Karten, sodass die Vereinigung der $U_{i}$ den\\
topologischen Raum X überdeckt. Zu zwei Karten ( $U_{i}, h_{i}$ ) und ( $U_{j}, h_{j}$ ) heißt
...
Definition 2.2 (Riemannsche Fläche) Eine Riemannsche Fläche ist ein hausdorffscher topologischer Raum mit einer komplexen Struktur. Eine Riemannsche Fläche heißt kompakt, wenn der zugrundeliegende topologische Raum kompakt ist.

Beispiele 2.3 Die komplexe Ebene $\mathbb{C}$, die obere Halbebene $\mathbb{H}=\{z \in \mathbb{C}$ : $\operatorname{Im}(z)>0\}$ und die Einheitskreisscheibe $\triangle=\{z:|z|<1\}$ sind Riemannsche Flächen, gegeben durch eine Karte, ebenso $\mathbb{C}^{*}=\mathbb{C} \backslash\{0\}$ und $\triangle^{*}=\triangle \backslash\{0\}$ die punktierte Ebene (bzw. punktierte Einheitskreisscheibe).

Beispiel 2.4 (Der Torus) Als zugrundeliegender topologischer Raum sei $X= S^{1} \times S^{1}$ gegeben, wobei $S^{1}=\{z \in \mathbb{C}:|z|=1\}$. Zu $w_{1}, w_{2} \in \mathbb{C}^{*}$ mit $\frac{w_{2}}{w_{1}} \notin \mathbb{R}$ definieren wir eine Riemannsche Fläche wie folgt. Sei (für $t_{1}, t_{2} \in \mathbb{R}$ )
...
Seien für $i \in I$ topologische Räume $X_{i}$ gegeben und für $j \in J \subset \mathbb{N}$ seien topologische Räume $U_{j}$ zusammen mit Abbildungen $\varphi_{1}: U_{j} \rightarrow X_{i_{1}}$ und $\varphi_{2}$ :

\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-010}
 
\caption{Abbildung 2.2: Der Torus als Quotient von \$\textbackslash mathbb\{C}\$\}\end{center}
\end{figure}

$U_{j} \rightarrow X_{i_{2}}$ gegeben, wobei $i_{1}$ und $i_{2}$ von $j$ abhängen. Sind die Bilder von $\varphi_{1}$ und $\varphi_{2}$ offen und die Abbildungen $\varphi_{1}, \varphi_{2}$ für alle $j \in J$ Homöomorphismen auf ihr Bild, so nennt man den topologischen Raum $X=\left(\coprod_{i \in I} X_{i}\right) / \sim$, mit der Äquivalenzrelation
...
das Verkleben von $X_{i}$ längs der $U_{j}$. Wir versehen $X$ vermöge der Abbildung $\coprod_{i \in I} X_{i} \longrightarrow X$ mit der Quotiententopologie. Die folgende Aussage ist unmittelbare Konsequenz der Definitionen.

Proposition 2.5 Sind die $X_{i}$ und $U_{j}$ offene Teilmengen von $\mathbb{C}$, die Inklusionsabbildungen $\varphi_{1}$ und $\varphi_{2}$ für alle $j \in J$ biholomorph auf ihr Bild und der durch Verkleben entstehende Raum $X$ hausdorffsch, so ist $X$ eine Riemannsche Fläche.

Beispiel 2.6 Ausgangsflächen seien $X_{1}=\mathbb{C}$ und $X_{2}=\mathbb{C}$. Die Verklebefläche ist $U_{1}=\mathbb{C}^{*}$ mit
...
Beispiel 2.7 Wir wählen $X_{1}, X_{2}$ und $U_{1,2}$ wie in Beispiel 2.4, aber $\varphi_{2}(z)=z$. Der resultierende topologische Raum ist nicht hausdorffsch, also keine Riemannsche Fläche.

Beispiel 2.8 Wir betrachten nochmal den Torus mit Perioden 1 und $i$ aus Beispiel 2.4. Wir wollen ihn nun durch Verkleben konstruieren. Sei
...
wie in Abbildung 2.3 und $T^{*}$ die Verklebung von $\coprod_{i=1}^{3} X_{i}$ entlang der offenen

\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-011}
 
\caption{Abbildung 2.3: Der Torus durch Verkleben}
\end{center}
\end{figure}

Mengen $U_{2}^{+}, U_{2}^{-}, U_{3}^{+}, U_{3}^{-}$wie folgt. Seien
...
Biholomorphe Selbstabbildungen einer Riemannschen Fläche $X$ (auch Biholomorphismen oder Automorphismen von $X$ genannt) bilden eine Gruppe,\\
die wir mit $\operatorname{Aut}(X)$ bezeichnen. Im Beispiel der elliptischen Kurve am Ende von Abschnitt 2.1 ist $(x, y) \mapsto(x,-y)$ ein Automorphismus von $X$, der elliptische Involution genannt wird.
...
Definition 2.10 (Verzweigungsindex) Sei $f: X \rightarrow Y$ eine holomorphe Abbildung zwischen Riemannschen Flächen, $a \in X$ und ( $U, g$ ) und ( $V, h$ ) Karten von Umgebungen von a bzw. von $f(a)$ mit $g(a)=0$ und $h(f(a))=0$. Der Verzweigungsindex von $f$ bei a ist definiert als
...
Nach dem Identitätssatz ist $e_{a}(f)<\infty$, falls $f$ nicht in einer Umgebung von $a$ konstant ist. Aus diesem Satz folgt auch, dass falls $f$ in einer Umgebung von $a$ konstant ist, so ist $f$ auf der ganzen Zusammenhangskomponente von $X$, welche $a$ enthält konstant. Eine holomorphe Abbildung $f: X \longrightarrow Y$ heißt nirgends konstant, falls $e_{a}(f)<\infty$ für alle $a \in X$ ist.

Wir benutzen Begriff der Ordnung, um eine Normalform für holomorphe Abbildungen herzuleiten.

Proposition 2.11 Sei $f: X \rightarrow Y$ holomorph und $a \in X$ mit $n=e_{a}(f)<\infty$. Dann gibt es Karten $(U, g)$ von $X$ und $(V, h)$ von $Y$, so dass $h \circ f \circ g^{-1}=z^{n}$ ist.

Beweis: Wähle zunächst $\operatorname{Karten}(\widetilde{U}, \widetilde{g})$ und $(V, h)$ mit $\widetilde{g}(a)=0, h(f(a))=0$ und $f(\widetilde{U}) \subset V$ ). Wir wollen aus der Potenzreihe
...
folgt die Behauptung. Es verbleibt noch zu zeigen, dass diese $n$-te Wurzel nicht nur eine formale Potenzreihe sondern in der Tat eine holomorphe Funktion auf einer Umgebung von Null ist (Übung).

Korollar 2.12 (Offenheitssatz) Eine nirgends konstante, holomorphe Abbildung ist offen.

Beweis: Für alle $n \in \mathbb{N}$ ist das Bild der offenen Kreisscheibe $\triangle_{r}$ unter $z \longmapsto z^{n}$ eine offene Kreisscheibe mit Radius $r^{n}$.
...
Satz 2.13 Sei $X$ zusammenhängend und $A$ analytisch. Dann ist $A=X$ oder $A$ ist lokal endlich in $X$.

Lemma 2.14 Sei $U \subseteq \mathbb{C}$ ein Gebiet und $f: U \longrightarrow \mathbb{C}$ holomorph und $a \in U$ ein Häufungspunkt der Nullstellenmenge von $f$. Dann gibt es eine Umgebung von a, auf der $f$ identisch verschwindet.

Beweis: Wir leiten die Aussage aus dem bekannten Satz, dass holomorphe Funktionen lokal in Potenzreihen entwickelbar sind, her. Sei also
...
Beweis des Satzes 2.13. Sei $M$ die Menge der Häufungspunkte von $A$. Diese ist abgeschlossen und $M \subset A$. Anwenden des Lemmas auf einen Punkt $a \in M$ ergibt, dass es eine offene Umgebung von $a$ gibt, die ganz in $M$ liegt. Also ist $M$ offen. Folglich ist $M=X$ oder $M=\emptyset$. Damit hat jeder Punkt in $a \in A$ eine offene Umgebung $U_{A}$ mit $A \cap U_{A}=\{a\}$ und damit ist $A$ lokal endlich.

Korollar 2.15 Seien $f_{1}, f_{2}: X \longrightarrow Y$ zwei holomorphe Abbildungen und $X$ zusammenhängend. Wenn $A=\left\{a \in X: f_{1}(a)=f_{2}(a)\right\}$ einen Häufungspunkt besitzt, so ist $f_{1}=f_{2}$. Insbesondere ist jede nicht-konstante holomorphe Abbildung offen.

Beweis: Die Menge $A$ ist abgeschlossen und analytisch, die Behauptungen folgen unmittelbar aus dem Satz.

\section {3 Überlagerungen und Fundamentalgruppen}
...
\section {1 Überlagerungen}
Seien $X$ und $Y$ Riemannsche Flächen. Wir setzen in diesem Abschnitt generall $X$ und $Y$ als nichtleer und zusammenhängend voraus. In Proposition 2.11haben wir jede nicht-konstante holomorphe Abbildung $f: X \rightarrow Y$ lokal in $X$ in geeigneten Karten als $z \mapsto z^{n}$ beschrieben. Der im folgenden definierte Begriff der Überlagerung beschreibt holomorphe Abbildungen lokal aus der Sichtweise von $Y$. In manchen Quellen wird dieser verzweigte Überlagerung genannt. Der Begriff der Überlagerung aus der Topologie korrespondiert zu dem unten definierten Begriff einer unverzweigten Überlagerung.

\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-016}
 
\caption{Abbildung 3.1: Eine (verzweigte) Überlagerung}
\end{center}
\end{figure}

\section {Definition 3.1 (Eigentliche und endliche Abbildungen, Überlagerungen)}
...
Beweis: Sei $A \subset X$ abgeschlossen und $b \in Y \backslash f(A)$. Sei $V$ eine Umgebung von $b$ mit kompakter Hülle $\bar{V}$. Dann ist $f^{-1}(\bar{V}) \cap A$ kompakt. Es gilt stets $f\left(f^{-1}(\bar{V}) \cap A\right)=\bar{V} \cap f(A)$ und diese Menge ist als stetiges Bild einer kompakten Menge wieder kompakt. Folglich ist $V \backslash(\bar{V} \cap f(A))$ eine Umgebung von $b$, die $f(A)$ nicht trifft.
...
mit einem $n_{i}=e_{x_{i}}(f) \in \mathbb{N}$ gilt. Indem wir zunächst $V$ durch den Schnitt $V \cap \bigcap_{i=1}^{d} f\left(\widetilde{U}_{i}\right)$ und dann $\widetilde{U}_{i}$ durch $U_{i}=\widetilde{U}_{i} \cap f^{-1}(V)$ ersetzen, können wir $f\left(U_{i}\right)=V$ sicherstellen. Da $f$ eigentlich ist, gibt es zu jedem $i$ ein $r_{i}$, sodass $h^{-1}$ auf $\Delta_{r_{i}}$ und $g_{i}^{-1}$ auf $\Delta_{r_{i}^{1 / n} i}$ definiert ist. Wir nehmen $r$ als das Minimum der $r_{i}$ und schließlich $V=h^{-1}\left(\Delta_{r}\right)$ sowie $U_{i}=g_{i}^{-1}\left(\Delta_{r^{1 / n}}\right)$.\\
...
Da $f$ eine Überlagerung ist, ist diese Abbildung lokal konstant, also konstant, da $Y$ zusammenhängend ist. Nach Voraussetzung ist die Konstante endlich, da $I=I_{y}$ in einem Punkt endlich ist. Es fehlt noch, die Eigentlichkeit zu zeigen. Sei dazu $K \subset Y$ kompakt. Für jedes $y \in Y$ gibt es Umgebungen $V_{y}$ und $U_{y, i}$\\
aus der Definition von Überlagerung. Da $Y$ lokal kompakt ist, finden wir auch offene Umgebungen $V_{y}^{\prime}$ von $y$, so dass $\overline{V_{y}^{\prime}}$ kompakt und in $V_{y}$ enthalten sind. Da $K$ kompakt ist, reichen endlich viele $V_{1}^{\prime}, \ldots, V_{r}^{\prime}$ um $K$ zu überdecken. Es reicht nun zu zeigen, dass $f^{-1}\left(\overline{V_{i}^{\prime}}\right)$ für alle $i=1, \ldots, r$ kompakt ist. Denn die endliche Vereinigung von Kompakta ist kompakt und $f^{-1}(K) \subset \bigcup_{i=1}^{r} f^{-1}\left(\overline{V_{i}^{\prime}}\right)$ ist als abgeschlossene Menge in einem Kompaktum ebenfalls kompakt. Dafür reicht es zu zeigen, dass $\left.f\right|_{U_{i j}}: U_{i j} \rightarrow V_{i}$ eigentlich ist. Dies kann man, da die Karten $h$ und $g_{i}$ Homöomorphismen sind, genauso gut für $h \circ f \circ g_{i}^{-1}$ nachprüfen. Die Abbildung $z \mapsto z^{n}$ ist aber eigentlich, denn das Urbild einer kompakten Menge ist beschränkt - da das Urbild von $B_{r}(0)$ in $B_{\sqrt[n]{r}}(0)$ liegt und abgeschlossen, also kompakt.
...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-019}
 
\caption{Abbildung 3.2: Eine Homotopie zwischen \$\textbackslash gamma\_\{0}\$ und $\gamma_{1}$\}\end{center}
\end{figure}
...
eine Homotopie zwischen $\gamma_{0}$ und $\gamma_{2}$. Man beachte die Stetigkeit der Abbildung $h$ bei $t=1 / 2$. \(\square\)
...
Beweis: Sei $\omega_{0}=\gamma_{3} \cdot\left(\gamma_{2} \cdot \gamma_{1}\right)$ und $\omega_{1}=\left(\gamma_{3} \cdot \gamma_{2}\right) \cdot \gamma_{1}$.\\
...
Definition 3.9 Ein Weg $\gamma$ von a nach a heißt geschlossen oder Schleife. Der Weg $\gamma^{-1}(t)=\gamma(1-t)$ heißt umgekehrter Weg. Mit $\pi_{1}(X, a, b)$ bezeichnen wir die Menge der Homotopieklassen von Wegen von a nach b.

Definition und Proposition 3.10 Die Homotopieklassen von Wegen von a nach a bilden eine Gruppe
...
Beweis: Neutrales Element ist $\gamma(t):=a$, inverses Element ist der umgekehrte Weg. Ist $\eta$ ein Weg von $a$ nach $b$, so definiert

$$
\gamma \mapsto \eta \cdot \gamma \cdot \eta^{-1}: \pi_{1}(X, a) \rightarrow \pi_{1}(X, b)
$$

den gewünschten Isomorphismus. \(\square\)

Definition 3.11 (Einfach zusammenhängend) Ein zusammenhängender topologischer Raum $X$ heißt einfach zusammenhängend, falls $\pi_{1}(X, a)=\{i d\}$ für ein $a \in X$ ist.

Proposition 3.12 Ist $X \subset \mathbb{C}$ sternförmig, so ist $X$ einfach zusammenhängend.\\
Beweis: Sei $a$ der Referenzpunkt des Sterns und $\gamma$ ein Weg. Dann ist

$$
h(s, t)=t \gamma(s)+(1-t) a
$$

eine Homotopie von $\gamma$ zum konstanten Weg am Punkt $a$. \(\square\)

Oftmals ist es nützlich, einen Weg lokal zu modifizieren. Dazu verwendet man folgenden Zerlegungssatz.

Proposition 3.13 Sei $\gamma: I \rightarrow X$ ein Weg und $U=\left\{U_{i}\right\}$ eine offene Überdeckung von $X$. Dann gibt es eine Zerlegung $0=s_{0}<s_{1}<\ldots<s_{n}=1$, sodass jeder Teilweg $\gamma\left(\left[s_{j}, s_{j+1}\right]\right)$ in einem $U_{i}$ enthalten ist.

Die Beweisidee veranschaulicht das Bild, die Details sind eine einfache Übung.

\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-020}
...
Beweis: Überdecke $X$ durch Wahl von je einer Kreisscheibe $U_{a}$ für jeden Punkt $a \in A$ mit der Eigenschaft $U_{a} \cap A=\{a\}$ und durch das Komplement $X \backslash A$. Wir wenden nun den Satz an und nehmen durch Zusammenfassen von Teilwegen an, dass kein Teilweg $\gamma\left(\left[s_{k}, s_{k+1}\right]\right)$ ausschließlich in einem $U_{a}$ liegt. Folglich liegt kein Teilungspunkt in $A$. Jeder Teilweg in einem $U_{a}$ trifft maximal einen Punkt in $A$. Wir ersetzen ihn durch einen anderen Weg in $U_{a}$, der $A$ nicht trifft. Da $U_{a}$ sternförmig, also einfach zusammenhängend ist, sind diese Wege homotop.

Korollar 3.15 Die Zahlenkugel $\mathbb{P}_{\mathbb{C}}^{1}$ ist einfach zusammenhängend.\\
Beweis: Wir können nach Korollar 3.14 annehmen, dass der Weg $\infty=\mathbb{P}_{\mathbb{C}}^{1} \backslash X_{1}$ vermeidet, also in $X_{1} \cong \mathbb{C}$ verläuft. Da $\mathbb{C}$ sternförmig ist, folgt die Behauptung aus Proposition 3.12.
...
Proposition 3.16 Ist $f: X \rightarrow Y$ eine unverzweigte Überlagerung, so sind Liftungen von Wegen eindeutig durch den Anfangspunkt bestimmt, d.h. zwei Liftungen $\gamma_{1}, \gamma_{2}$ des Weges $\gamma: I \rightarrow Y$ stimmen überein, sobald sie an einem Punkt $c \in I$ übereinstimmen. Außerdem ist $f$ unbegrenzt.
...
homotop ist.\\
Satz 3.26 Seien $a_{0} \ldots, a_{r} \in \mathbb{P}_{\mathbb{C}}^{1}$ paarweise verschieden mit $r \geq 1$. Es gibt einfache Schleifen $\gamma_{j}$ um die Punkte $a_{j}$ für $j=0, \ldots, r$, sodass
...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-027}
...
Proposition 3.29 Jede orientierbare triangulierbare topologische Fläche besitzt einen komplexen Atlas.

Beweis: Siehe z.B. Théorème 2.3 in Rey89.\\
Wir können nun das Hauptresultat des Abschnitts formulieren.\\
Satz 3.30 Jede kompakte Riemannsche Fläche $X$ ist homöomorph zu $S^{2}$ oder (als topologischer Raum) zur Verklebung wie in Abbildung 3.6 eines Polygons mit $2 g$ Kanten, wobei Kanten mit gleichem Symbol (bis auf Potenz) unter Beachtung der Pfeilrichtung identifiziert werden.
...
Beweis: Man pflastere einen zusammenhängenden und einfach zusammenhängenden Teil der Ebene mit den Dreiecken der Triangulierung. Man nummeriere Kanten am Rand des Polygons, die auf $X$ identifiziert waren mit den Kanten-Symbolen $a_{i}$ und $a_{i}^{-1}, i \in I_{K}$. Man identifiziere weitere Ecken, die dem gleichen Punkt in $X$ entsprechen, mit demselben Ecken-Symbol $P_{i}$ mit $i \in I_{E}$. Dieses Polygon wird mit den folgenden Modifikationen in Normalform gebracht.

\begin{enumerate}
  \item Falls es nur zwei Seiten $a$ und $a^{-1}$ gibt, so ist $X \sim S^{2}$.
  \item Falls es mehr als zwei Seiten gibt, sorgt man zunächst dafür, dass nur ein Ecksymbol, sagen wir $p$ auftritt. Denn gibt es noch ein weiteres Symbol $q$, so sorgt das Abtrennen und Neuverkleben in Figur (untenstehend)
\end{enumerate}

\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-030}
 
\caption{Abbildung 3.6: Kanonische Zerschneidung}
\end{center}
\end{figure}

dafür, dass die Anzahl der Ecken mit Symbol $q$ um eins erniedrigt und die mit Symbol $p$ um eins erhöht wird.

\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-030(1)}
...
\begin{enumerate}
  ...
  \item Zu jedem Paar verschachtelter Kanten kann man durch die Wiederver-
\end{enumerate}

\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-031}
 
\caption{Abbildung 3.8: Schritt 4 von Satz 3.30}
\end{center}
\end{figure}

klebung wie skizziert in Abbildung 3.9 ein paar aufeinanderfolgender verschachtelter Kanten herstellen.

\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-031(1)}
...
\begin{enumerate}
  \setcounter{enumi}{5}
  \item Die gestrichelten Abschnitte bleiben in Schritt 5 unberührt. Man kann also die Schritte 3 und 5 wiederholen, bis alle verschachtelten Paare aufeinanderfolgen. \(\square\)
\end{enumerate}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-032}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-032(1)}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-034}
...
\includegraphics[max width=\textwidth, center]{2025_10_06_374d4cfa8b1948eb4c41g-066}\\
...
\includegraphics[max width=\textwidth, center]{2025_10_06_374d4cfa8b1948eb4c41g-070}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-092}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-093}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-094}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-096}
...
\includegraphics[max width=\textwidth, center]{2025_10_06_374d4cfa8b1948eb4c41g-097}
...
\includegraphics[max width=\textwidth, center]{2025_10_06_374d4cfa8b1948eb4c41g-108}\\
...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-109}
...
\includegraphics[max width=\textwidth, center]{2025_10_06_374d4cfa8b1948eb4c41g-111}\\
\includegraphics[max width=\textwidth, center]{2025_10_06_374d4cfa8b1948eb4c41g-111(1)}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-117}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-122}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-126}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-127}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-129}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_10_06_374d4cfa8b1948eb4c41g-130}
