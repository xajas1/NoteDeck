...
\begin{REM}{LIE-H15-01-13}{Problem mit Implicit-Function-Theorem Ansatz}
...
\begin{REM}{LIE-H15-01-14}{Matrix Lie Gruppen sind abgeschlossene Untergruppen von $\GL(n;\C)$}
...
\begin{REM}{LIE-H15-01-15}{Starke Konvergenzeigenschaft von meisten Matrix Lie Gruppen}
...
$$
That is to say, $\operatorname{Sp}(n)$ is the group of $2 n \times 2 n$ matrices that preserve both the inner product and the bilinear form $\omega$.
...
\begin{PROOF}{LIE-H15-01-36}{P: Charakterisierung von $\U(2n)$-Elementen in $\Sp(n)$}
Fix some $U$ in $\mathrm{U}(2 n)$. Then for $z$ and $w$ in $\mathbb{C}^{2 n}$, we have, on the one hand,
$$
...
The preceding result can be used to give a different perspective on the definition of $\mathrm{Sp}(n)$, as follows. 
...
\begin{PROOF}{LIE-H15-01-39}{P: Charakterisierung von Eigenwerten/vektoren von $\U(2n)$ in $\Sp(n)$}
Consider $U$ in $\operatorname{Sp}(n ; \mathbb{C}) \cap \mathrm{U}(2 n)$, choose an eigenvector for $U$, normalized to be a unit vector, and call it $u_{1}$. Since $U$ preserves the norms of vectors, the eigenvalue $\lambda_{1}$ for $u_{1}$ must be of the form $e^{i \theta_{1}}$, for some $\theta_{1} \in \mathbb{R}$. If we set $v_{1}=J u_{1}$, then since $J$ is conjugate linear and commutes with $U$ (Proposition 1.5), we have
$$
...
\begin{PROOF}{LIE-H15-01-44}{P: Invarianz-Eigenschaften von $J$ auf komplexen Unterräumen}
If $w \in V^{\perp}$ then for all $z \in V$, we have
$$
...
Most of the other groups we have considered are noncompact. 
...
\begin{REM}{LIE-H15-01-50}{Äquivalenz WZSH und ZSH für MLG}
...
\begin{PROOF}{LIE-H15-01-53}{P: $id$-WZSH-Komponente ist normale Untergruppe in MLG}
...
\begin{PROOF}{LIE-H15-01-56}{P: $\GL(n;\C)$ ist WZSH für $n\geq 1$}
We make use of the result that every matrix is similar to an upper triangular matrix (Theorem A.4). That is to say, we can express any $A \in M_{n}(\mathbb{C})$ in the form $A=C B C^{-1}$, where
$$
...
\begin{PROOF}{LIE-H15-01-58}{P: $\SL(n;\C)$ ist WZSH für $n\geq 1$}
...
\begin{PROOF}{LIE-H15-01-60}{P: $\U(n)$ ist WZSH für $n\geq 1$}
By Theorem A.3, every unitary matrix has an orthonormal basis of eigenvectors, with eigenvalues having absolute value 1 . Thus, each $U \in \mathrm{U}(n)$ can be written as $U_{1} D U_{1}^{-1}$, where $U_{1} \in \mathrm{U}(n)$ and $D$ is diagonal with diagonal entries $e^{i \theta_{1}}, \ldots, e^{i \theta_{n}}$. We may then define
$$
...
$$
It is easy to see that $U(t)$ is in $\mathrm{U}(n)$ for all $t$, and $U(t)$ connects $U$ to $I$.
\end{PROOF}
...
\begin{PROOF}{LIE-H15-01-62}{P: $\SU(n)$ ist WZSH für $n\geq 1$}
A slight modification of this argument, as in the proof of Proposition 1.12, shows that $\mathrm{SU}(n)$ is connected.
\end{PROOF}
...
\begin{PROOF}{LIE-H15-01-67}{P: $\SU(2)$ ist EZSH}
Exercise 5 shows that $\mathrm{SU}(2)$ may be thought of (topologically) as the three-dimensional sphere $S^{3}$ sitting inside $\mathbb{R}^{4}$. It is well known that $S^{3}$ is simply connected; see, for example, Proposition 1.14 in [Hat].
\end{PROOF}
...
\begin{PROOF}{LIE-H15-01-74}{P: $\SO(3)$ und $\R P^3$ sind homöomorph und $\SO(3)$ damit nicht EZSH}
...
An important topic for us will be the relationship between the groups $\operatorname{SU}(2)$ and SO(3), which are almost, but not quite, isomorphic. Specifically, we now show that there exists a Lie group homomorphism $\Phi: \mathrm{SU}(2) \rightarrow \mathrm{SO}(3)$ that is two-to-one and onto. 
...
\begin{PROOF}{LIE-H15-01-82}{P: LG Homomorphismus zwischen $\SU(2)$ und $\SO(3)$}
Consider the space $V$ of all $2 \times 2$ complex matrices $X$ which are self-adjoint (i.e., $X^{*}=X$ ) and have trace zero. Elements of $V$ are precisely the matrices of the form
$$
...
\end{aligned}
$$
since the trace is invariant under conjugation. Thus, each $\Phi_{U}$ preserves the inner product trace $\left(X_{1} X_{2}\right) / 2$ on $V$. It follows that the map $U \mapsto \Phi_{U}$ is a homomorphism of SU (2) into the group of orthogonal linear transformations of $V \cong \mathbb{R}^{3}$, that is, into $\mathrm{O}(3)$. 

Since $\mathrm{SU}(2)$ is connected (Proposition 1.13), $\Phi_{U}$ must actually lie in $\mathrm{SO}(3)$ for all $U \in \mathrm{SU}(2)$. 

Thus, $\Phi$ (i.e., the map $U \mapsto \Phi_{U}$ ) is a homomorphism of $\mathrm{SU}(2)$ into $\mathrm{SO}(3)$, which is easily seen to be continuous. Since $(-I) X(-I)^{-1}=X$, we see that $\Phi_{-I}$ is the identity element of $\operatorname{SO}(3)$.
\end{PROOF}
...
\begin{PROOF}{LIE-H15-01-84}{P: LG Homomorphismus zwischen $\SU(2)$ und $\SO(3)$ ist 2-1 und Onto}
Exercise 16 shows that the kernel of $\Phi$ is precisely the set $\{I,-I\}$. To see that $\Phi$ maps onto $\mathrm{SO}(3)$, let $R$ be a rotation of $V \cong \mathbb{R}^{3}$. By Exercise 14, there exists an "axis" $X \in V$ such that $R$ is a rotation by some angle $\theta$ in the plane orthogonal to $X$. If we express $X$ in the form
...
\begin{REM}{LIE-H15-01-87}{Explizite Berechnung von LG Homomorphismus zwischen $\SU(2)$ und $\SO(3)$}
...
\begin{PROOF}{LIE-H15-02-06}{P: Ungleichungen der Hilbert-Schmidt-Norm}
The first of these inequalities is the triangle inequality for $\mathbb{C}^{n^{2}}$ and the second follows from the Cauchy-Schwarz inequality (Exercise 1).
\end{PROOF}
...
\begin{PROOF}{LIE-H15-02-08}{P: Exponentialabbildung konvergiert überall in $M_n(\C)$}
In light of $\|X Y\| \leq\|X\|\|Y\|$, we see that
$$
...
\begin{PROOF}{LIE-H15-02-10}{P: Elementare Eigenschaften der Exponentialabbildung}
Point 1 is obvious and Point 2 follows from taking term-by-term adjoints of the series for $e^{X}$. 

Points 3 and 4 are special cases of Point 5. 

To verify Point 5, we simply multiply the two power series term by term, which is permitted because both series converge absolutely. Multiplying out $e^{X} e^{Y}$ and collecting terms where the power of $X$ plus the power of $Y$ equals $m$, we obtain
$$
...
\begin{PROOF}{LIE-H15-02-12}{P: Induzierte glatte Kurve durch Exponentialabbildung}
Differentiate the power series for $e^{t X}$ term by term. This is permitted because, for each $j$ and $k,\left(e^{t X}\right)_{j k}$ is given by a convergent power series in $t$, and one can differentiate a power series term by term inside its radius of convergence (e.g., Theorem 12 in Chapter 4 of [Pugh]).
\end{PROOF}
...
We consider here methods for exponentiating general matrices. 

A special method for exponentiating $2 \times 2$ matrices is described in Exercises 6 and 7. 

\begin{CONC}{LIE-H15-02-13}{Berechnung von Matrix-Exponentialabbildung für invertierbare Matrizen}
Suppose that $X \in M_{n}(\mathbb{C})$ has $n$ linearly independent eigenvectors $v_{1}, \ldots, v_{n}$ with eigenvalues $\lambda_{1}, \ldots, \lambda_{n}$. Let $C$ be the $n \times n$ matrix whose columns are $v_{1}, \ldots, v_{n}$ and let $D$ be the diagonal matrix with diagonal entries $\lambda_{1}, \ldots, \lambda_{n}$. Then $X=C D C^{-1}$. It is easily verified that $e^{D}$ is the diagonal matrix with diagonal entries $e^{\lambda_{1}}, \ldots, e^{\lambda_{n}}$, and thus, by Proposition 2.3, we have
$$
...
\begin{CONC}{LIE-H15-02-14}{Berechnung von Matrix-Exponentialabbildung für nilpotente Matrizen}
Meanwhile, if $X$ is nilpotent (i.e., $X^{k}=0$ for some $k$ ), then the series that defines $e^{X}$ terminates.
\end{CONC}


\begin{CONC}{LIE-H15-02-15}{Berechnung von Matrix-Exponentialabbildung durch Zerlegung in Diagonal und Nilpotent}
Finally, according to Theorem A.6, every matrix $X$ can be written (uniquely) in the form $X=S+N$, with $S$ diagonalizable, $N$ nilpotent, and $S N=N S$. Then, since $N$ and $S$ commute,
$$
e^{X}=e^{S+N}=e^{S} e^{N}
$$
\end{CONC}
...
\begin{CONC}{LIE-H15-02-17}{Exponentialabbildung für DGLs}
...
Fig. 2.1 Curves of the form $t \mapsto e^{t X_{4}} \mathbf{v}_{0}$ (left) and $t \mapsto e^{t X_{5}} \mathbf{v}_{0}$ (right)
\end{CONC}
...
\begin{PROOF}{LIE-H15-02-19}{P: Logarithmus in komplexer Analysis}
...
\begin{CONC}{LIE-H15-02-21}{Konvergenzkriterium für Logarithmus}
...
\begin{PROOF}{LIE-H15-02-23}{P: Wohldefiniertheit von Log-Abbildung und Bedingungen für Investierung von Exp-Abbildung}
Since $\left\|(A-I)^{m}\right\| \leq\|(A-I)\|^{m}$ and since the series (2.7) has radius of convergence 1, the series (2.9) converges absolutely for all $A$ with $\|A-I\|<1$. The proof of continuity is essentially the same as for the exponential.
...
\begin{CONC}{LIE-H15-02-24}{Relevanz der Hilbert-Schmidt-Schranke für Invertierbarkeit}
...
\begin{PROOF}{LIE-H15-02-26}{P: Abschätzung von $\log (I+b)-B$}
Note that
$$
...
\begin{CONC}{LIE-H15-02-27}{Näherung von $\log (I+B)$}
...
Theorem 2.11 (Lie Product Formula). 
...
\begin{PROOF}{LIE-H15-02-30}{P: Lie-Produkt Regel}
If we multiply the power series for $e^{\frac{X}{m}}$ and $e^{\frac{Y}{m}}$, all but three of the terms will involve $1 / m^{2}$ or higher powers of $1 / m$. Thus,
$$
...
\begin{PROOF}{LIE-H15-02-32}{P: Determinante des Exponentialabbildung}
If $X$ is diagonalizable with eigenvalues $\lambda_{1}, \ldots, \lambda_{n}$, then $e^{X}$ is diagonalizable with eigenvalues $e^{\lambda_{1}}, \ldots, e^{\lambda_{n}}$. Thus, trace $(X)=\sum_{j} \lambda_{j}$ and
$$
...
Theorem 2.14 (One-Parameter Subgroups). 
...
\begin{PROOF}{LIE-H15-02-35}{B: Darstellungen von 1-Parameter Untergruppen von $\GL(n;\C)$ durch Exp-Abbildung}
The uniqueness is immediate, since if there is such an $X$, then $X=\left.\frac{d}{d t} A(t)\right|_{t=0}$. To prove existence, let $U$ be as in Lemma 2.15, which is an open set in $\operatorname{GL}(n ; \mathbb{C})$. The continuity of $A$ guarantees that there exists $t_{0}>0$ such that $A(t) \in U$ for all $t$ with $|t| \leq t_{0}$. Define
$$
...
\begin{PROOF}{LIE-H15-02-37}{P: Eindeutige Existenz von Wurzeln für Elemente im Bild von Exp-Abbildung von hinreichend kleiner Umgebung von Ursprung}
It is evident that $C$ is a square root of $B$ and that $C$ is in $U$. To establish uniqueness, suppose $C^{\prime} \in U$ satisfies $\left(C^{\prime}\right)^{2}=B$. Let $Y=\log C^{\prime}$; then $\exp (Y)= C^{\prime}$ and
$$
...
\begin{PROOF}{LIE-H15-02-39}{P: Exp-Abbildung ist $C^\infty(M_n(\C),M_n(\C))$}
...
\begin{CONC}{LIE-H15-02-40}{Polar-Dekomposition für Matrizen}
...
We wish to establish a similar polar decomposition first for $\operatorname{GL}(n ; \mathbb{C})$ and then for various subgroups thereof.
\end{CONC}
...
Suppose now that $A$ is an invertible $n \times n$ matrix. We wish to write $A$ as $A=U P$ where $U$ is unitary and $P$ is self-adjoint and positive. We will then write the selfadjoint, positive matrix $P$ as $P=e^{X}$ where $X$ is self-adjoint but not necessarily positive.
...
\begin{PROOF}{LIE-H15-02-43}{P: Eigenschaften von Dekompositionen von invertierbaren Matrizen}
For the existence of the decomposition in Point 1, note that if $A=U P$, then $A^{*} A=P U^{*} U P=P^{2}$. Now, for any matrix $A$, the matrix $A^{*} A$ is self-adjoint. If, in addition, $A$ is invertible, then for all nonzero $v \in \mathbb{C}^{n}$, we have
$$
...
The existence and uniqueness of the decomposition in Point 2 are proved in precisely the same way as in Lemma 2.18, with the logarithm function (which is a bijection between $(0, \infty)$ and $\mathbb{R}$ ) replacing the square root function. The same sort of reasoning shows that for any self-adjoint $X$, the matrix $e^{X}$ is self-adjoint and positive.
...
We then note that the square root operation on self-adjoint, positiv matrices $P$ is also continuous, since $P^{frac{1}{2}}$ may be computed as $\exp(\log(P)/2)$. It is then evident from the formulas for $U$ and $P$, that these quantities depend continuously on $A$.
\end{PROOF}
...
\begin{PROOF}{LIE-H15-02-45}{P: Eindeutige selbstadjungierte Wurzel für selbstadjungierte positive Matrizen}
Since $Q$ has an orthonormal basis of eigenvectors, $Q$ can be written as
$$
...
\begin{PROOF}{LIE-H15-02-47}{P: Polar-Dekomposition von $\GL(n;\R),\SL(n;\C),\SO(n;\R)$}
If $A$ is real, then $A^{*} A$ is real and symmetric. Now, a real, symmetric matrix can be diagonalized over $\mathbb{R}$. Thus, $P$, which is the unique self-adjoint positive square root of $A^{*} A$ (constructed as in (2.11)), is real. Then $U=A P^{-1}$ is real and unitary, hence in $\mathrm{O}(n)$.
...
\begin{CONC}{LIE-H15-03-04}{Lie-Klammer als $XY-YX$ in einer assoziativen Algebra nach Jacobi-Indentität}
...
\begin{CONC}{LIE-H15-03-11}{Vorteile der Adjungierten Abbildung}
We can view ad (that is, the map $\left.X \mapsto \mathrm{ad}_{X}\right)$ as a linear map of $\mathfrak{g}$ into $\operatorname{End}(\mathfrak{g})$, the space of linear operators on $\mathfrak{g}$. The Jacobi identity is then equivalent to the assertion that $\mathrm{ad}_{X}$ is a derivation of the bracket:
$$
...
\begin{PROOF}{LIE-H15-03-13}{P: Adjungierte Abbildung als Lie Algebren Homomorphismus}
Observe that
$$
...
\begin{CONC}{LIE-H15-03-16}{Nachweis von Direkter Summen-Darstellung von Lie Algebren}
...
Although we will not have much occasion to use them, structure constants do appear frequently in the physics literature. 


\begin{CONC}{LIE-H15-03-18}{Relationen für Strukturkonstanten}
...
The first of these conditions comes from the skew symmetry of the bracket, and the second comes from the Jacobi identity.
\end{CONC}
...
\begin{CONC}{LIE-H15-03-22}{Einfach Lie Algebra als irreduzible nicht-kommutative Algebra}
...
\begin{CONC}{LIE-H15-03-27}{Ideal-Eigenschaften in abgeleiteten Reihen von Lie Algebren}
...
\begin{CONC}{LIE-H15-03-30}{Eigenschaften j-ter Kommutatoren und Nilpotent impliziert Lösbar}
...
\begin{CONC}{LIE-H15-03-34}{Inklusionsbedingung für Lie Algebra einer Matrix Lie Gruppe}
...
\begin{PROOF}{LIE-H15-03-36}{P: $e^X$ von Lie Algebren Elementen sind in Identität-Komponente der Lie Gruppe }
By definition of the Lie algebra, $e^{t X}$ lies in $G$ for all real $t$. However, as $t$ varies from 0 to $1, e^{t X}$ is a continuous path connecting the identity to $e^{X}$.
\end{PROOF}
...
\begin{PROOF}{LIE-H15-03-38}{P: Elementare Eigenschaften von Lie Algebren von Matrix Lie Gruppen}
For Point 1, we observe that, by Proposition 2.3,
$$
...
\begin{CONC}{LIE-H15-03-39}{Reelle und komplexe Lie Algebren für Matrix Lie gruppen}
...
\begin{PROOF}{LIE-H15-03-43}{P: Lie Algebren von kommutative Matrix Lie Gruppen sind kommutativ}
For any two matrices $X, Y \in M_{n}(\mathbb{C})$, the commutator of $X$ and $Y$ may be computed as
$$
...
\begin{EXE}{LIE-H15-03-54}{E: Lie Algebra der Heisenberg Gruppe $H$}
...
\begin{CONC}{LIE-H15-03-59}{Umgang mit induziertem LA Homomorphismus}
...
\begin{PROOF}{LIE-H15-03-58}{P: Induzierter LA Homomorphismus durch LG Homomorphismus zwischen Matrix Lie Gruppen}
The proof is similar to the proof of Theorem 3.20. Since $\Phi$ is a continuous group homomorphism, $\Phi\left(e^{t X}\right)$ will be a one-parameter subgroup of $H$, for each $X \in \mathfrak{g}$. Thus, by Theorem 2.14, there is a unique matrix $Z$ such that
$$
...
\begin{PROOF}{LIE-H15-03-62}{P: Kompositionalität von induzierten Lie Algebra Homomorphismen}
...
\begin{PROOF}{LIE-H15-03-64}{P: Induzierte Lie Algebra von (abgeschlossenen, normalen) Kern eines LG Homomorphismus}
The usual algebraic argument shows that $\operatorname{ker}(\Phi)$ is normal subgroup of $G$. Since, also, $\Phi$ is continuous, $\operatorname{ker}(\Phi)$ is closed. If $X \in \operatorname{ker}(\phi)$, then
$$
...
Definition 3.32 (The Adjoint Map). 
...
\begin{PROOF}{LIE-H15-03-68}{P: LG Homomorphismus von MLG in Gruppe der invertierbaren Homomorphismen der Lie Algebra}
Easy. Note that Point 1 of Theorem 3.20 guarantees that $\operatorname{Ad}_{A}(X)$ is actually in $\mathfrak{g}$ for all $X \in \mathfrak{g}$.
...
show that $\mathrm{Ad}: G \rightarrow \mathrm{GL}(\mathfrak{g})$ is continuous, and so is a Lie group homomorphism.
\end{PROOF}
...
\begin{PROOF}{LIE-H15-03-71}{P: Konsistenz der adjungierten Abbidlungsbegriffe}
By Point 3 of Theorem 3.28, ad can be computed as follows:
$$
...
\begin{PROOF}{LIE-H15-03-73}{P: $e^XYe^{-X}$ Formel für adjungierte Abbildung auf $M_n(\C)$}
We have proved, as a consequence of Theorem 3.28 and Proposition 3.34, the following result, which we will make use of later.
...
\begin{PROOF}{LIE-H15-03-76}{P: Komplexifizierte Lie Algebra und eindeutige Erweiterung der Lie-Klammer}
The uniqueness of the extension is obvious, since if the bracket operation on $\mathfrak{g}_{\mathbb{C}}$ is to be bilinear, then it must be given by
$$
...
\begin{PROOF}{LIE-H15-03-78}{P: Charakterisierung von Komplexifizierung von reellen Lie Algebren durch Matrizen der Form $X+iY$}
...
\begin{PROOF}{LIE-H15-03-82}{P: Eindeutige Erweiterung von reellen Lie Algebra Homomorphismen in komplexe LA nach Komplexifizierung (Universelle Eigenschaft der Komp.)}
The unique extension is given by $\pi(X+i Y)=\pi(X)+i \pi(Y)$ for all $X, Y \in \mathfrak{g}$. It is easy to check that this map is, indeed, a homomorphism of complex Lie algebras.
\end{PROOF}
...
\begin{PROOF}{LIE-H15-03-86}{P: Charakterisierung von Inklusion in MLG und Lie Algebren durch Logarithmus}
Let us think of $M_{n}(\mathbb{C})$ as $\mathbb{C}^{n^{2}} \cong \mathbb{R}^{2 n^{2}}$ and let $D$ denote the orthogonal complement of $\mathfrak{g}$ with respect to the usual inner product on $\mathbb{R}^{2 n^{2}}$. Consider the map $\Phi: M_{n}(\mathbb{C}) \rightarrow M_{n}(\mathbb{C})$ given by
$$
\Phi(Z)=e^{X} e^{Y},
$$
...
\begin{CONC}{LIE-H15-03-87}{Rolle der $\epsilon$-Umgebung für Inklusions-Charakterisierung}
...
\begin{PROOF}{LIE-H15-03-89}{P: Inklusion Charakterisierung für Lie Algebren durch logarithmisierte Folgen}
For any $t \in \mathbb{R}$, we have $\left(t /\left\|Y_{m}\right\|\right) Y_{m} \rightarrow t Y$. Note that since $B_{m} \rightarrow I$, we have $\left\|Y_{m}\right\| \rightarrow 0$. Thus, we can find integers $k_{m}$ such that $k_{m}\left\|Y_{m}\right\| \rightarrow t$. We have, then,
$$
...
\begin{PROOF}{LIE-H15-03-91}{P: Exponential Abbildung homöomorph für hinreichend kleine Umgebungen von $0$ und $I$}
Let $\varepsilon$ be such that Theorem 3.42 holds and set $U=U_{\varepsilon} \cap \mathfrak{g}$ and $V= V_{\varepsilon} \cap G$. The theorem implies that $\exp$ takes $U$ onto $V$. Furthermore, $\exp$ is a homeomorphism of $U$ onto $V$, since there is a continuous inverse map, namely, the restriction of the matrix logarithm to $V$.
\end{PROOF}
...
\begin{PROOF}{LIE-H15-03-93}{P: Lie Gruppe mit k-dim reeller Lie Algebra als glatte eingebettete $k$-Untermannigfaltigkeit von $M_N(\C)$}
...
\begin{PROOF}{LIE-H15-03-96}{P: Lie Algebra von Matrix Lie Gruppen ist der Tangentialraum am Einheitselement}
...
\begin{PROOF}{LIE-H15-03-100}{P: Kontrollierbarkeit von $A(s)A(t)^{-1}-I$}
We note that
$$
\begin{aligned}
...
\begin{PROOF}{LIE-H15-03-102}{P: Charakterisierung von Gleichheit von MLG Homomorphismen bei ZSH Definitionsbereich via LA Homomorphismen}
Let $g$ be any element of $G$. Since $G$ is connected, Corollary 3.47 tells us that every element of $G$ can be written as $e^{X_{1}} e^{X_{2}} \cdots e^{X_{m}}$, with $X_{j} \in \mathfrak{g}$. Thus,
$$
\begin{aligned}
...
\begin{PROOF}{LIE-H15-03-104}{P: Stetige Homomorphismen zwischen Matrix Lie Gruppen sind glatt}
For all $g \in G$, we write nearby elements $h \in G$ as $h=g e^{X}$, with $X \in \mathfrak{g}$, so that
$$
...
\begin{PROOF}{LIE-H15-03-106}{P: ZSH Matrix Lie Gruppen mit kommutativen Lie Algebren sind kommutativ}
Since $\mathfrak{g}$ is commutative, any two elements of $G$, when written as in Corollary 3.47, will commute.
\end{PROOF}
...
\begin{PROOF}{LIE-H15-03-108}{P: Identität-Komponenten von Matrix Lie Gruppen in $M_n(\C)$ sind Matrix Lie Gruppen mit übereinstimmenden Lie Algebren}
