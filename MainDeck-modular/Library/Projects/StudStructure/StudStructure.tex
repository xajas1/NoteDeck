...
% 1) Kurzbefehl: \td{...}  (optional mit Farbwunsch)
\newcommand{\td}[2][blue]{{\color{#1}#2}}
% Nutzung:
% \td{dieser Text ist blau}
% \td[red]{dieser Text ist rot}
% \td[HTML]{dieser Text ist DodgerBlue}  % vorher Farbe definieren: \definecolor{HTML}{HTML}{1E90FF}
...
\usepackage{tikz}
\usetikzlibrary{decorations.pathreplacing,arrows.meta,positioning,babel}
\usepackage{tikz-cd}
...
\StructurePicAutoB{}{%
  \begin{outline}
    ...
    \1 1. Inklusion von Vereinigung in Urbild
    \2 \td{Hopf-Rinow} auf $\tilde M$: $\exp_{\tilde m_i}(B(0_{\tilde m_i},r))=\tilde U_i$
    \2 Alle Elemente in Komponenten $\tilde U_i$ können durch Kurven von Länge kleiner $r$ mit Faserpunkten $\tilde m_i$ verbunden werden und projizierte Kurve hat gleiche Länge kleiner $r$, also liegt der Punkt auch in $U$; damit liegen jene Punkte im Urbild der Projektion von $U$
    \1 Lokale Diffeomorphie
    \2 Für beliege Startgeschwindigkeiten $v, \tilde v$ gilt $\pi \circ \exp _{\tilde{m}_{i}} \tilde{v}=\exp _{m} \circ d \pi(\tilde{v})$ (\td{lokale Isometrie} gibt korrespondierende Geodätische $\pi\circ\tilde \gamma$)
    \2 Diagramm kommutiert 
    $${\shorthandoff{"}
    \begin{tikzcd}[ampersand replacement=\&]
      B(0_{\tilde{m}_i}, r) \arrow[r, "{\exp_{\tilde{m}_i}}"] \arrow[d, "{d\pi}"']
        \& B(\tilde{m}_i, r) = \tilde U_i \arrow[d, "{\pi}"] \\
      B(0_m, r) \arrow[r, "{\exp_m}"']
        \& B(m, r) = U
    \end{tikzcd}
    }$$ 
    Damit sind $\pi|_{\tilde U_i}$ sind diffeomorph
    \1 Umgekehrte Inklusion von Urbild in Vereinigung
    \2 Wähle $\tilde q$ im Urbild und $q=\pi(\tilde q)\in U$
    \2 Existiert eine eindeutige minimale BLP Geodätische von $q$ nach $m$ mit Länge $s<r$
    \2 Existiert \td{korrespondierende} Geodätische $\tilde \gamma$ in Komponente $\tilde U_i$ von $\tilde q$, die wegen geodätischer Vollständigkeit auf $\R$ definiert ist
    \2 Da $\pi\circ \tilde\gamma=\gamma$ folgt $\tilde \gamma(s)=\tilde m$ und damit $\tilde q\in \tilde U_i$
...
\StructurePicAutoB{}{%
  \begin{outline}
    \1 Widerspruch: Verbinde Faserpunkte in Überlagerung (ZSH) und definiere eindeutige existierende geschlossene Kurve bzgl Basispunkt, zu welchem Verbindungskurve der eindeutige Lift ist. 
    \1 Geschlossene Kurve ist homotop zu konstante Basispunkt-Kurve (EZSH)
    \1 Homotopie-Abbildung gibt in $t$ parametrisiert gecshlossene Kurven im Basisraum bzgl Basispunkt
    \1 Lifte bzgl Parameter $t$ die Kurven $c_t$ in Überlagerungsraum
    \2 Alle gelifteten Kurven haben Startpunkt gleich erster Faserpunkt
    \1 Definiere Kurve im Überlagerungsraum für $s=1$ durch induzierten Lift $t\mapsto \tilde c_t(1)$ (Untersuche: Wo landen die Endpunkte der Lifts?)
    \2 $\tilde c_t(1)$ ist konstant (Kurve ist stetig und Faser als Wertebereich von $\tilde c_t(1)$ ist diskret)
    \2 $\tilde c_0(1)=\tilde\gamma(1)=q_1$ und $\tilde c_1(1)=q_0$
    \1 Widerspruch
  \end{outline}
}
...
\textbf{Statement: }{Klassifikation von EZSH vollständigen Mannigfaltigkeiten mit konstanter Schnittkrümmung}.
...
\StructurePicAutoB{}{%
  \begin{outline}
    \1 Setting: Basispunkt und Geodätische zu Tangentialvektor in $M$ sowie parallele Vektorfelder zu Startvektoren und entsprechende Jacobifelder \td{mit Darstellung $Y_i(t)= d_{tv}\exp _m(tu_i)$}
    \1 Fall: $K=0$
    \2 Krümmung verschwindet und Jacobigleichung vereinfacht sich, sodass explizit Darstellung von Jacobifelder durch parallele Vektorfelder bestimmt werden kann
    \2 $\exp:T_m M \to M$ ist \td{lokale} Isometrie
    \3 Differential von $\exp$ kann durch Jacobifelder und in diesem Fall durch parallele Vektorfelder dargestellt werden $$Y_i(t)=tU_i(t)$$, wodurch die Isometrie zwischen Tangentialräumen (Eigenschaft der parallelen Vektorfelder) nachgewiesen werden kann
    \2 $\exp$ ist damit eine Überlagerung (lok. Iso und Tangentialraum geodätisch vollständig)
    \2 $\exp$ ist Isometrie (Tangentialraum ZSH und $M$ EZSH)
    \1 Fall $K=-1$
    \2 Analog zu $K=0$ vereinfach sich Jacobigleichung, sodass $$Y_i(t)=\sinh(t)U_i(t)$$
    \2 Konstruiere lokale Isometrie $\mathbb{H}\to M$ via Isometrie von Tangentialräumen und vollständigkeit von $\mathbb{H}$
    \3 $$F=\exp_m\circ f^{-1}\circ \exp_{\tilde m}^{-1}$$
    mit $f:T_m M\to T_{\tilde m}\mathbb{H}$ Isometrie und Übertragung der Geodätischen, parallelen Vektorfelder und Jacobifelder von $M$ mit $$\tilde Y_i=\sinh(t)\tilde U_i(t)$$
    \3 \td{Nachweis lokale Isometrie durch Berechnung der Metrik unter Differential}
    \1 Fall $K=1$
    \2 Konstruiere lokale Isometrien $F: S^n\backslash \{-\tilde m\}\to M$ und $F_1:S^n\backslash \{-\tilde m'\}\to M$ für einen Punkt $\tilde m'=\tilde m,-\tilde m$, die miteinander verträglich sind
    \3 $F$ analog zu $K=-1$
    \3 $$
    F_{1}=\left.\exp _{m^{\prime}} \circ d F\right|_{\tilde{m}^{\prime}} \circ \exp _{\tilde{m}^{\prime}}^{-1}
    $$
    ist lokale Isometrie, dass $dF$ und damit die Inverse bei $\tilde m'$ isometrisch ist, 
    \3 Abbildungen und deren Differentiale stimmen bei $\tilde m'$ überein
    \2 Geodätische (nicht durch $\tilde m$) werden von beiden Abbildungen auf gleiche Geodätische in $M$ abgebildet (Lokale Isometrie und Übereinstimmung der Anfangsdaten)
    \2 Damit stimmen $F$ und $F_1$ auf Punkten außerhalb der verbindenden Geodätischen zwischen $\tilde m, -\tilde m'$ überein, und können stetig fortgesetzt werden bis auf $-\tilde m'$ bzw. $ -\tilde m$
    \2 Zusammenkleben zu einer lokalen Isometrie zwischen $S^n$ und $M$
    \2 Standardargument via \td{Überlagerung}
  \end{outline}
}
...
\subsection{Volumen Riemannischer Mannigfaltigkeiten}



\textbf{Statement: }{Disjunkte Zerlegung von Mannigfaltigkeit bzgl Schnittort}.
...
\StructurePicAutoB{}{%
  \begin{outline}
    \1 Verinigung ergibt $M$
    \2 Klar aus Vollständigkeit, da alle Punkte durch minimale Geodätische verbunden werden können
    \1 Disjunkt durch Widerspruch
    \2 Punkt $\exp$ von $U_m$ und im Schnittort
    \3 Minimale Geodätische $c$ bis $x$ und etwas darüber hinaus
    \3 Minimale Geodätische $\gamma$ bis $x$, aber nicht weiter
    \2 Da $\gamma\neq c$ muss insbesondere die Geschwindigkeit bei $m$ verschwieden sein
    \2 Konstruiere Geodätische $\tau$, welche bis $1$ mit $\gamma$ und ab $1$ mit $c$ übereinstimmt. Dann hat die Geodätische minimal, aber hat einen Knick  
  \end{outline}
}
...
\textbf{Statement: }{Schnittorte sind Nullmengen}.
Für jedes $m \in M$ ist der Schnittort Cut $(m)$ eine Nullmenge.

\StructurePicAutoB{}{%
  \begin{outline}
    $\partial U_m$ ist Nullmenge in $T_mM$ und wird durch $\exp$ stetig auf Nullmenge abgebildet
  \end{outline}
}
...
\textbf{Statement: }Berechnung des Volumens ausgehend von Punkt.

\StructurePicAutoB{}{%
  \begin{outline}
    \1 ONB $\{u,e_2,…,e_n\}$ von $T_mM$ und Jacobifelder $Y_i$ längs $c$ zu $e_i$ mit 
    $$\left(d \exp _{m}\right)_{t u} e_{i}=\frac{1}{t} Y_{i}(t)$$
    \1 Pullback Volumenform längs Exponentialabbildung berechnen $$\left(\exp _m^* v_g\right)_{t u}\left(u, e_2, \ldots, e_n\right)=t^{-(n-1)} \sqrt{\operatorname{det}\left(g\left(Y_i(t), Y_j(t)\right)\right)}$$
    \1 Trafo von $dx_1…dx_n$ zu $t^{n-1}dt\ du$
    $$\exp _m^* v_g=J(u, t) d x_1 \cdots d x_n=J(u, t) t^{n-1} d t d u$$
    \1 Schnittort ist Nullmenge
    \1  \[
    vol(M,g)=\int_{U_m}\exp_m^*v_g.
    \]
    \1 $$\operatorname{vol}(M, g)=\int_{S^{n-1}} \int_0^{\rho(u)} J(u, t) t^{n-1} d t d u$$
  \end{outline}
}
...
\textbf{Statement: }Volumenberechnung von Sphäre

\StructurePicAutoB{}{%
  \begin{outline}
    \1 $Y_i(t)=\sin(t)E_i(t)$ mit $E_i$ parallele Vektorfelder mit $\rho(u)=\pi$
    \1 $$\begin{aligned}
    ...
    \end{aligned}
    $$
    \1 Induktiv kann man zeigen, dass
    $$
...
\textbf{Statement: }Volumenberechnung des hyperbolischen Raumes

\StructurePicAutoB{}{%
  \begin{outline}
    \1 $$
    \operatorname{vol}\left(B_{m}(R)\right)=\operatorname{vol}\left(S^{n-1}, \text { can }\right) \int_{0}^{R} \td{\sinh ^{n-1}(r)} d r
    $$
...
\textbf{Statement: } Volumenberechnung komplex-projektiver Raum

\StructurePicAutoB{}{%
  \begin{outline}
    \1 Wähle orthonormale Basis $\left\{u, e_{2}, \ldots\right\}$ von $T_{m} \mathbb{C}^{n}$ mit $e_{2}=J u$. 
    \1 $$Y(s)=d \pi(\tilde{Y}(s))=\sin (s) \underbrace{d \pi(\tilde{U}(s)}_{=: U(s)})$$
    $$Y(s)=d \pi(\tilde{Y}(s))=\sin (s) d \pi(\tilde{U}(s))=\cos (s) \sin (s) J c^{\prime}(s)$$
    \1 Damit sind Jacobifelder gegeben durch
    $$
    \begin{aligned}
    ...
    \end{aligned}
    $$
    \1 Schnittort Abstand ist $\pi\backslash 2$
    \1 $$
...
\textbf{Statement: }\td{{Variation der Determinante}.
...
\textbf{Statement: }{Volumen Darstellung von Integral über symmetrische Bilinearform auf $\R^n$}.
...
\textbf{Statement: }{Darstellung des Volumen kleiner Bälle durch Skalarkrümmung}.
...
\StructurePicAutoB{}{%
  \begin{outline}
    \1 Radius kleiner Abstand zum Schnittort kann Volumen berechnet werden durch
    $$
    \operatorname{vol} B_{m}(r)=\int_{S^{n-1}} \int_{0}^{r} J(u, t) t^{n-1} d t d u
    $$
    \1 Taylorn von $J(u,t)$ bis 2. Ordnung
    $$
    ...
    $$
    \2 Geodätische mit ONB in $T_m M$ und entsprechende Jacobifelder
    \2 Jacobigleichung nach $s$ bei $s=0$ ableiten, effektiv nur Terme mit $Y_i$, rest verschwindet
    $$
    ...
    $$
    \2 Taylorn 2. Ordnung von $Y_i$ bei $s=0$ liefert ($Y_i'(0)=Y_i''=0$)
    $$
    Y_{i}(t)=t E_{i}(t)-\sum_{j} \frac{t^{3}}{6} R\left(u, e_{i}, u, e_{j}\right) E_{j}(t)+o\left(t^{3}\right)
    $$
    \3 Mit $Y_i$ parallele Vektorfelder längs c, $Y_i(0)=\frac{D^i}{d t^i} Y(0)$. Dann gilt für alle $k$
    $$
    Y(t)=\sum_{i=0}^k \frac{t^i}{i!} Y_i(t)+o\left(t^k\right)
    $$ 
    \2 Metrikkomponenten berechnen
    $$g\left(Y_{i}(t), Y_{j}(t)\right)=t^{2} \delta_{i j}-\frac{t^{4}}{3} R\left(u, e_{i}, u, e_{j}\right)+o\left(t^{4}\right)$$
    ...
    $$
    \3 Als Matrix aufgefasst $$\det (M(t))=t^{(n-1)2}\det (1-\frac{t^2}{3}R(u,e_i,u,e_i)+o(t^2))$$
    und 1. Orndung Taylorn der restlichen Determinante ergibt $tr R(u,e_i,u,e_i)=Ric(u,u)$ 
    \1 Näherung einsetzen
    $$
    \begin{aligned}
    ...
    \end{aligned}
    $$
    Dabei Formel für Integral von Ricci als symmetrischer BLF über $S^{n-1}$ verwenden
  \end{outline}
}
...
\textbf{Statement: }{Bishop-Gunther Theorem}.
...
\StructurePicAutoB{}{%
  \begin{outline}
    \1 Trafo zwischen Radial Jacobian für allgemeine Jacobifeld Basis und Jacobifeld ONB längs Geodätische durch $C\in GL(n)$
    $$
    \begin{aligned}
    ...
    \end{aligned}
    $$
    mit $\det C^2=\det g(Y_i',Y_j')(0)$
    \2 Definiere für fixe Radialrichtung $u$  
    $$f(t)=J(u,t)$$
    und $D(t)=\det (Y_i(t),Y_j(t))$
    \1 Zu parallel transportierter ONB längs Geodätische existiert für $r$ kleiner als der Abstand zum Schnittort ein Jacobifelder $Y^r_i$ mit $Y_i^r(0)=0, Y_i^r(r)=E_i(0)$
    \2 $$Y^r_i(t)=d\exp _m|_{tu}(tv)$$ als Variationsfeld einer geodätischen Variation
    \1 Hilfsmittel für abschließende Abschätzung
    \2 \begin{equation*}
    \frac{f^{\prime}(r)}{f(r)}=\sum_{i=2}^{n} I\left(Y_{i}^{r}, Y_{i}^{r}\right)-\frac{n-1}{r} \tag{12}
    \end{equation*}
    \3 \td{TD}
    \2 Für Jacobifelder $Y$ und Vektorfelder $X$ längs minimaler Geodätischer mit übereinstimmenden Endwerten gilt:
    $$I(Y,Y)\leq I(X,X)$$
    Daraus ergibt sich 
    $$
    \sum_{i=2}^{n} I\left(Y_{i}^{r}, Y_{i}^{r}\right) \leq \sum_{i=2}^{n} I\left(X_{i}^{r}, X_{i}^{r}\right)
    $$
    im Fall 1 und im Fall 2 auf der Modelmannigfaltigkeit ($\tilde X$ Jacobifeld)
    $$
    \sum I\left(\tilde{Y}_{i}^{r}, \tilde{Y}_{i}^{r}\right) \geq \sum I\left(\tilde{X}_{i}^{r}, \tilde{X}_{i}^{r}\right)
    $$
    \3 \td{TD}
    \2 Für Modelmannigfaltigkeiten mit konstanter Krümmung $a$ wird die Jacobigleichung mit Anfangsdaten $Y_i'(0)=E_i(0)$ gelöst durch
    $$Y_i^r(t)=Y_i(t)=\frac{s_a(t)}{s_a'(t)}E_i(t)$$
    $$
    s_a(t):= \begin{cases}\sin \sqrt{a} t & a>0 \\ t & a=0 \\ \sinh \sqrt{-a} t & a<0\end{cases}
    $$
    Man vergleicht anschließend mit Vektorfeldern 
    $$X_i^r(t)=\frac{s(t)}{s'(t)}E_i(t)$$
  \end{outline}
}
\StructurePicAutoB{}{%
\begin{outline}
    \1 Abschätzung $f(s)\leq f_a(s)$ für $Ric \geq (n-1)ag$ und Herleitung des Volumen-Vergleichs
    \2 $$
    \frac{f^{\prime}(r)}{f(r)} \leq \frac{f_{a}^{\prime}(r)}{f_{a}(r)}
    $$
    \3 $I(X_i^r,X_i^r)$ partiell integrieren, aufsummieren von $i=2$ bis $n$ und von oben Abschätzen durch negatives Integral (Vorraussetzung an Ricci) gibt
    $$\sum I(X_i^r,X_i^r)\leq (n-1)\frac{s'(r)}{s(r)}$$
    \3 C.1 vereinfachen durch C.2 und A.1.1. ergibt durch C.3 die Differentialgleichung mit Gleichheit für Modellraum
    \2 Integrieren bis $s$ kleiner als der Abstand zum Schnittort liefert $$\log f(s)\leq \log f_a(s)$$ und damit $$f(s)\leq f_a(s)$$
    \2 Abschließende Volumenberechnung via Radialjacobian liefert $$vol (B_m(r))\leq V^a(r)$$
    \1 Abschätzung $f(s)\geq f_b(s)$ und Herleitung des Volumenvergleichs für $K\leq b$
    \2 Für eines der eindeutigen Jacobifelder $Y(t)=Y^r_i(t)=\sum y_i(t)E_i(t)$ gilt
    ...
    \2 Definiere auf EZSH Mannigfaltigkeit mit konstanter Schnittkrümmung $b$ 
    $$\tilde Y(t)=\sum y_i(t)\tilde E_i(t)\quad \tilde X_i^r(t)=\frac{s(t)}{s(r)}\tilde E_i(t)$$ 
    längs einer Geodätischen $\tilde c$ der Länge $r$ und $\tilde E_i$ PVF und ONB ($\tilde X$ ist ein Jacobifeld mit übereinstimmenden Endwerten zu $\tilde Y$) es gilt 
    $$
    ...
    $$
    \2 C.1 vereinfacht durch Übergang auf Modelmannigfaltigkeit (B.1 und B.2) und C.2 ergibt die Abschätzung
...
\textbf{Statement: }{Milnor-Wolf Theorem}.
