...
\usepackage{paracol}   % echte zweispaltige Umgebung mit Seitenumbruch
\usepackage{mdframed}  % hübscher, breakable Rahmen
\usepackage{xparse,xstring,graphicx}
\usepackage[most]{tcolorbox}

% Voraussetzungen: \usepackage[most]{tcolorbox}, \usepackage{xparse,xstring,graphicx}
% Längen (falls noch nicht gesetzt):
% \newlength{\StructureLeftWidth}   \setlength{\StructureLeftWidth}{0.26\textwidth}
% \newlength{\StructureGap}         \setlength{\StructureGap}{1.5em}

% Fallback-Parser (falls dein echter schon existiert, bleibt der hier ungenutzt)
\makeatletter
\@ifundefined{ParseArtCaption}{%
  \newcommand{\ParseArtCaption}[2]{%
    \def#2{#1}% voller Pfad
    \global\def\StructCaption{}%
  }%
}{}
\makeatother

% \begin{StmtStruct}[<optional Bild>]{<Statement-Text>}
%   <Outline/Text>
% \end{StmtStruct}

\NewDocumentEnvironment{StmtStruct}{ O{} +m +b }{%
  \begingroup
  % --- Bild + Caption vorbereiten (optional) -------------------------------
  \def\fullpath{}\global\def\StructCaption{}%
  \def\maybeimg{#1}%
  \StrSubstitute{\maybeimg}{ }{}[\maybeimgNoSpace]%
  \IfStrEq{\maybeimgNoSpace}{}{%
    \let\hasimage\relax
  }{%
    \ParseArtCaption{\maybeimg}{\fullpath}%
    \def\hasimage{YES}%
  }%

  % --- Eine bruchfähige Box für alles (Statement + Structure) -------------
  \begin{tcolorbox}[
    enhanced, breakable,
    boxrule=0.4pt, colframe=gray, colback=white, sharp corners,
    before skip=0pt, after skip=.6\baselineskip,
    top=.6\baselineskip, bottom=.6\baselineskip,
    % vertikale Trennlinie zwischen Bild und Outline (in allen Box-Teilen)
    overlay={%
      \ifx\hasimage\relax\else
        \draw[gray!60]
          ([xshift=\dimexpr\StructureLeftWidth+\StructureGap/2\relax]interior.north west) --
          ([xshift=\dimexpr\StructureLeftWidth+\StructureGap/2\relax]interior.south west);
      \fi
    },
    % Wir unterscheiden ersten vs. folgende Box-Teile
    before upper={%
      \ifnum\value{tcbbreakpart}=0
        % ---------- ERSTER TEIL: Statement + Trenner + Start Structure -----
        \noindent\textbf{Statement: }#2\par\medskip
        \tcbline % horizontale Linie innerhalb der Box
        \ifx\hasimage\relax
          \noindent\textbf{Structure:}\par\vspace{.4\baselineskip}%
          \begin{minipage}[t]{\linewidth}%
        \else
          \begin{minipage}[t]{\StructureLeftWidth}%
            \textbf{Structure:}\par\vspace{.4\baselineskip}%
            \centering
            \includegraphics[width=\linewidth]{\fullpath}\\[.4ex]%
            {\footnotesize\itshape \StructCaption}%
          \end{minipage}%
          \hspace{\StructureGap}%
          \begin{minipage}[t]{\dimexpr\linewidth-\StructureLeftWidth-\StructureGap\relax}%
        \fi
      \else
        % ---------- FOLGETEILE: nur rechte Spalte in voller Breite ---------
        \begin{minipage}[t]{\linewidth}%
      \fi
    },
    after upper={%
  ...
  % ---- Body: deine Outline / Struktur-Text (bricht über Seiten) ----------
  #3
}{%
  \end{tcolorbox}%
  \endgroup
...
\usepackage{xparse,calc,xstring}


% --- Bruchfähige Structure-Box -----------------------------------------------
\usepackage[most]{tcolorbox}


% Basis: bruchfähige Side-by-Side-Box ohne sichtbaren Rahmen
...
% Bruchfähige Version MIT Bild/Caption links
\newcommand{\StructurePicAutoB}[2]{%
  \begingroup
    % ---- Auto-Caption-Parsing (deine funktionierende Logik) ----
    \def\fn{#1}%
    \StrSubstitute{\fn}{ }{}[\fnnospace]%
    \IfStrEq{\fnnospace}{}{%
      % kein Bild: auf Bildlose Variante ausweichen
      \StructureB{#2}%
    }{%
      \IfBeginWith{\fn}{Art/}{%
        \StrBehind[1]{\fn}{/}[\base]%
...
% Bruchfähige Version OHNE Bild (nur rechter Text, Box behält Look)
...
% Benötigt: \usepackage{xstring}

% Eingabe:
%   {}                                  → ohne Bild (fällt auf \Structure zurück)
%   ART0000__Artist__Title__Year.EXT    → Bild + Auto-Caption
%   Art/ART0000__Artist__Title__Year... → dito (Pfad wird nicht verdoppelt)
\newcommand{\StructurePicAuto}[2]{%
  \begingroup
    % 0) Eingabe prüfen: leere Eingabe oder nur Leerzeichen? → \Structure
    \def\fn{#1}%
    \StrSubstitute{\fn}{ }{}[\fnnospace]%
    \IfStrEq{\fnnospace}{}{%
      \Structure{#2}%
      ...
      % 3) Versuche Muster „…__Artist__Title__Year.ext“ zu parsen
      \StrCount{\base}{__}[\dbl]%
      \ifnum\dbl>0
        \StrBehind[1]{\base}{__}[\restA]%  Artist__Title__Year.ext
        \StrBefore [1]{\restA}{__}[\artistraw]%
        \StrBehind [1]{\restA}{__}[\restB]%  Title__Year.ext
        \StrBefore [1]{\restB}{__}[\titleraw]%
        \StrBehind [1]{\restB}{__}[\yearExt]% Year.ext
        \StrBefore [1]{\yearExt}{.}[\yearraw]%
        % Unterstriche hübsch machen (verhindert Missing-$)
        \StrSubstitute{\artistraw}{_}{ }[\artist]%
        \StrSubstitute{\titleraw}{_}{ }[\title]%
        \edef\year{\yearraw}%
      \else
        % Fallback: nutze Basisname ohne Endung als Titel
        \StrBefore[1]{\base}{.}[\title]%
      \fi
      ...
      % 5) Rendern
      \StructurePic{\fullpath}{\caption}{#2}%
    }%
  \endgroup
}

% Resthöhe MESSEN (muss VOR der tcolorbox passieren!)
...
% Bild anhand der VORHER gemessenen Resthöhe setzen
...
\newlength{\StructureSep}
\setlength{\StructureSep}{\dimexpr\StructureLeftWidth+\StructureGap/2\relax}
\newsavebox{\StmtStructLeftBox}
\newlength{\StmtStructLeftHeight}
...
\begin{StmtStruct}[Art/ART0050__Paul_Klee__Hoffmannesque_Scene__1921.jpg]
{Fundamentaltheorem der Riemannischen Geometrie. Sei $(M,g)$ …}
{
\begin{outline}
  \1 Koszul-Gleichung …
  \2 …
...
% Ohne Bild:
\begin{StmtStruct}[]{Lokale Isometrie: Zeige, dass lokal …}{
  \begin{outline}
    \1 Variation im Tangentialraum …
    \1 …
  \end{outline}
}
\end{StmtStruct}
...
\StructurePicAuto{Art/ART0011__Paul_Klee__Dittlsam__1918.JPG}{%
\begin{outline}
  Christoffel-Symbole treten als Korrekturfaktoren ...
\end{outline}
}
...
\StructurePicAutoB{ART0050__Paul_Klee__Hoffmannesque_Scene__1921.jpg}
{%
\begin{outline}
...
\StructurePic{ART0011__Paul_Klee__Dittlsam__1918.JPG}{}
{%
\begin{outline}
...
\begin{StmtStruct}[Art/ART0050__Paul_Klee__Hoffmannesque_Scene__1921.jpg]
{Exponentialabbildung als lokaler Diffeomorphismus. Die Abbildung exp : $\Omega \rightarrow M$ für $\Omega:=\{v \in$ $\left.T M: 1 \in I_{v}\right\}$ ist differenzierbar. Für $p \in M$ ist die Abbildung
$$
\begin{aligned}
