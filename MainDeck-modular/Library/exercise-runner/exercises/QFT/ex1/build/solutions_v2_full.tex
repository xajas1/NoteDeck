...
\paragraph{Lösung.}
Zunächst konvertieren wir 1 kg in GeV. In natürlichen Einheiten gilt $\hbar = c = 1$, was bedeutet, dass Energie, Masse und Impuls dieselben Einheiten haben. Die Umrechnung von kg in GeV erfolgt über die Ruheenergie eines Kilogramms, $E = mc^2$, wobei $c^2$ in GeV umgerechnet wird:
...
Nun konvertieren wir 1 s in $\mathrm{GeV}^{-1}$. In natürlichen Einheiten, wo $c = 1$, ist die Lichtgeschwindigkeit gleich 1, was bedeutet, dass Zeit und Länge dieselben Einheiten haben. Daher entspricht 1 Sekunde einer Länge von 1 Meter. Die Umrechnung erfolgt über die Beziehung:
...
\[
G_N = 6.67 \times 10^{-11} \, \mathrm{m}^3 \, \mathrm{kg}^{-1} \, \mathrm{s}^{-2} \times \left( \frac{1.519 \times 10^{24} \, \text{GeV}}{1} \right)^2 \times \left( \frac{1}{5.61 \times 10^{26} \, \text{GeV}} \right)
\]
...
Diese Umformung zur Divergenzform ist korrekt, da der Ausdruck \(\Psi^* \nabla^2 \Psi - \Psi \nabla^2 \Psi^*\) durch Anwendung der Produktregel und der Definition der Divergenz in die Form \(\nabla \cdot (\Psi^* \nabla \Psi - \Psi \nabla \Psi^*)\) umgeschrieben werden kann. Dies entspricht der Divergenz eines Vektorfeldes, was die Formulierung der Kontinuitätsgleichung ermöglicht.
...
Diese Herleitung gilt unter der Annahme, dass der Hamiltonoperator \(\hat{H}\) zeitunabhängig ist. Falls der Hamiltonoperator zeitabhängig ist, muss die Herleitung angepasst werden, da dann zusätzliche Terme in der Ableitung der Exponentialoperatoren auftreten. Im Heisenberg-Bild bleibt der Hamiltonoperator \(\hat{H}_H = \hat{H}\) konstant, wenn er zeitunabhängig ist.
