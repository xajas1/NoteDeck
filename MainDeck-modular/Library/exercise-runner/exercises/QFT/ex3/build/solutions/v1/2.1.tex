Um die Gleichung $\gamma^{\mu \dagger} = \gamma^0 \gamma^\mu \gamma^0$ zu verifizieren, beginnen wir mit der Definition der Dirac-Matrizen. Die Dirac-Matrizen erfüllen die Clifford-Algebra:
...
\[
\gamma^{\mu \dagger} = (\gamma^0 \gamma^\mu \gamma^0)^\dagger = \gamma^0 (\gamma^\mu)^\dagger \gamma^0
\]
...
wobei $S(\Lambda)$ die Spinor-Repräsentation der Lorentztransformation ist. Das adjungierte Spinor transformiert dann wie folgt:
...
\[
\Gamma = \gamma^0 \Gamma^\dagger \gamma^0 = \bar{\Gamma}
\]
