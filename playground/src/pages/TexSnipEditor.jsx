import React, {
  ...
  const TexSnipEditor = forwardRef(({ splitState, onMetaChange, onNewUnit }, ref) => {
    ...
    useEffect(() => {
  const handleQuickSnip = (ctypShortcut) => {
    setCtyp(ctypShortcut)
    ctypRef.current = ctypShortcut
    handleSubmit()
  }
  ...
  const listener = (e) => {
    ...
    if (!(isCmd && isShift) || e.repeat) return
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", padding: "1rem", height: "100%" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: "0.3rem", fontSize: "0.75rem", marginBottom: "0.5rem" }}>
          ...
          <div><label>Content:</label><input ref={contentInputRef} value={content} onChange={e => setContent(e.target.value)} style={{ width: "100%" }}/></div>
          <div><label>CTyp:</label><select value={ctyp} onChange={e => setCtyp(e.target.value)} style={{ width: "100%" }}>{["DEF","PROP","THEO","LEM","KORO","REM","OTH","PROOF","EXA","STUD","CONC","EXE","MOT","STR"].map(k => <option key={k} value={k}>{k}</option>)}</select></div>
        </div>
